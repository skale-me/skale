...
console.log("\n" + "# textFile persist textFile persist crossProduct lookupStream");
...
co(function *() {
	...
	lsource[0] = v_ref[0];
	lsource[1] = v_ref[1];
	lsource[1] = ut.crossProduct(lsource[1], lsource[0]);
	var loc = [lsource[1].filter(function (e) {return (e[0] == key)})];
	
	dsource[0] = uc.textFile("/tmp/cedricartigue/v0").map(function(e) {return e.split(" ").map(parseFloat)});
	dsource[0] = dsource[0].persist();
	yield dsource[0].count();
	fs.appendFileSync("/tmp/cedricartigue/v0", "\n1 2\n");
	dsource[1] = uc.textFile("/tmp/cedricartigue/v1").map(function(e) {return e.split(" ").map(parseFloat)});
	dsource[1] = dsource[1].persist();
	yield dsource[1].count();
	fs.appendFileSync("/tmp/cedricartigue/v1", "\n1 2\n");
	dsource[1] = dsource[1].crossProduct(dsource[0]);
	var dist = [];
	var out = dsource[1].lookup(key, {stream: true});
