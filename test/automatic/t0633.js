...
console.log("\n" + "# textFile sample persist reduceStream");
...
co(function *() {
	...
	lsource[0] = v_ref[0];
	lsource[0] = ut.sample(lsource[0], uc.worker.length, withReplacement, frac, seed);
	var loc = [lsource[0].reduce(reducer, [0, 0])];
	
	dsource[0] = uc.textFile("/tmp/cedricartigue/v0").map(function(e) {return e.split(" ").map(parseFloat)});
	dsource[0] = dsource[0].sample(withReplacement, frac);
	dsource[0] = dsource[0].persist();
	yield dsource[0].count();
	fs.appendFileSync("/tmp/cedricartigue/v0", "\n1 2\n");
	var dist = [];
	var out = dsource[0].reduce(reducer, [0, 0], {stream: true});
