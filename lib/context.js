...
function SkaleContext(arg) {
  ...
  this.runTask = function(task, callback) {
    ...
    //dlog(task.start, 'task size for worker ' + wid + ':', str.length);
    //log('task', str);
        ...
        function runNext() {
          while (busy <  nworker && index < tasks.length) {
            ...
            self.runTask(tasks[index++], function (err, res, task) {
              ...
              totalFiles += n;
              totalSize += size;
              dlog(task.start, 'part', task.pid, 'from worker-' + res.workerId, '(' + complete + '/' + tasks.length + '), shuffle out:', n, 'files,', (size/(1<<20)).toFixed(3), 'MB,');
