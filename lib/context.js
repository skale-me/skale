...
function SkaleContext(arg) {
	...
	this.parallelize = function (localArray, nPartitions) { return skale.parallelize(this, localArray, nPartitions);};
	this.range = function (start, end, step, nPartitions) { return skale.range(this, start, end, step, nPartitions);};
	this.textFile = function (file, nPartitions) {return new skale.TextFile(this, file, nPartitions);};
	this.lineStream = function (stream, config) {return new skale.Stream(this, stream, 'line', config);};
	this.objectStream = function (stream, config) {return new skale.Stream(this, stream, 'object', config);};
			...
			findNodes(stage, function(nodes) {
				...
				for (var i = 0; i < stage.dependencies.length; i++)
					for (var j = 0; j < stage.dependencies[i].partitions.length; j++)
						stage.shufflePartitions[pid++] = new skale.Partition(stage.id, pid, stage.dependencies[i].id, 
...
function Task(contextId, jobId, nodes, rddid, pid, action) {
	...
	this.run = function(done) {
		var pipeline = [], self = this, mm = this.mm, fs = this.lib.fs, action = this.action;
		var p = this.pid;
