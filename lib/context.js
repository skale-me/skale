    ...
    function runShuffleStage(stage, done) {
      findNodes(stage, function(nodes) {
        ...
        for (i = 0; i < stage.nShufflePartitions; i++)
          tasks.push(new Task(self.basedir, jobId, nodes, stage.id, i));

        function runNext() {
          var remain = tasks.length - index;
          if (remain) log('remaining tasks:', remain);
          var max = (remain < maxWorker ? remain : maxWorker) - busy;
          for (var i = 0; i < max; i++) {
            busy++;
            self.runTask(tasks[index++], function (err, res) {
