...
var Source = require('./dataset.js').Source;

var ml = {};
module.exports = ml;

ml.randomSVMData = function (sc, N, D, seed, nPartitions) {
	function getSplit(P, p, arg, task) {
		var n = Math.ceil(arg.N / P);
		var rng = new task.lib.ml.Random(arg.seed + p * n * (arg.D + 1));
		var i = 0, k = p * n, out = [];

		while (i < n && k++ < arg.N)
			out[i++] = task.lib.ml.randomSVMLine(rng, arg.D);
		return out;
	}
	return new Source(sc, getSplit, {N, D, seed}, nPartitions);
}
...
ml.LogisticRegression = function (sc, data, D, N, w) {
	var self = this;
	this.w = w || zeros(D);
	this.sc = sc;
...
ml.LinearRegression = function (sc, data, D, N, w) {
	var self = this;
	this.w = w || zeros(D);
	this.sc = sc;
...
ml.KMeans = function (sc, data, nClusters, initMeans) {
	var seed = 1;
