...
ml.StandardScaler = function() {
	...
	this.fit = function(points, done) {
		...
		points.aggregate(reducer, combiner, {sum: [], count: 0}).on('data', function(data) {
			...
			function reduce(acc, features) {
				for (var i in features) {
					if (acc.sum[i] == undefined) acc.sum[i] = 0;	// suboptimal initialization (number of features unknown ?)
					acc.sum[i] += Math.power(features[i] - acc.mean[i], 2);
				}
				acc.count++;
				return acc;				
			}
			...
			points.aggregate(reducer, combiner, {sum: [], count: data.count, mean: self.mean}).on('data', function(data) {
				self.std = [];
				for (var i in data.sum)
					self.std[i] = Math.sqrt(data.sum[i] / data.count);
				done();
			});
		});
	}
...
ml.LogisticRegression = function (points, weights) {
	var self = this;
	this.points = points;
	this.weights = weights;	// May be undefined on startup
	this.D;
	this.N;
