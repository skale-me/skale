		...
		function processBlock(p) {
			...
			lines.on("endNewline", function(lastLineComplete) {
				...
				if (shuffleLastLine) {
					...
					if (shuffleTo != wid)
						grid.request(worker[shuffleTo], {cmd: 'lastLine', args: {lastLine: shuffledLine, lid: lid, p: p}}, function(err) {if (err) throw new Error(err);});
					else
						rxLastLine = lastLine;
	...
	this.endStage = function(callback) {
		...
		if (fun_action) {
			...
			try {
				...
				throw new Error("Lineage tx shuffle " + transform[transform.length - 1].type + ": " + err);
			}
		}
	}
}
