...
module.exports.textFile = function(lid, grid, worker, state, node, RAM, transform, action, fun_action) {
	...
	var run = this.run = function(callback) {
		...
		processBlock(blockIdx, end);

		function end() {
			if (++blockIdx < blocks.length) processBlock(blockIdx, end);
			else self.endStage(callback);		
		}

		function processBlock(p, callback) {
			var lines = new Lines();
			state.locked = true;
			...
			lines.on("end", function() {
				...
				if (shuffleLastLine) {
					grid.request_cb(worker[shuffleTo], {cmd: 'lastLine', args: {lastLine: lastLine, lid: lid, p: p}}, function(err) {if (err) throw err;});					
				} else {
					self.tmp = [lastLine];
					if (persistent) self.save(0);
					self.pipeline(p);					
