...
function Source(grid, app, job, lid, state, transform, inLastStage) {
	var node = job.node;
	var RAM = app.RAM;
	var partitionMapper = {};
	this.tmp = [];
	...
	this.pipeline = function(p, head) {
		...
		if (inLastStage) job.action.pipeline(this.tmp, p, head);
	};
}

module.exports.stream = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);

 	var cbk;
 	var self = this;
 	var persistent = job.node[transform[0]].persistent;
 	var num = transform[0];
...
module.exports.parallelize = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);

	this.run = function(callback) {
		var input = job.node[transform[0]].args[0] || [];
 		var persistent = job.node[transform[0]].persistent;
...
module.exports.fromRAM = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);

	this.run = function(callback) {
		var input = app.RAM[job.node[transform[0]].id] || [];
		for (var p = 0; p < input.length; p++) {
...
module.exports.fromSTAGERAM = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);

	this.run = function(callback) {
		var input = job.node[transform[0]].transform.SRAM || [];
 		var persistent = job.node[transform[0]].persistent;
		for (var p = 0; p < input.length; p++) {
			var partition = input[p].data;
			for (var i = 0; i < partition.length; i++) {
...
module.exports.randomSVMData = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);
...
module.exports.mongo = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);
...
module.exports.textFile = function(grid, app, job, lid, state, transform, inLastStage) {
	Source.call(this, grid, app, job, lid, state, transform, inLastStage);
	...
	function processBlock(bid) {
		...
		function shuffleLine(shuffledLine) {
			...
			if (shuffleTo != app.wid) {
				grid.send(app.worker[shuffleTo].uuid, {cmd: 'lastLine', args: {lastLine: shuffledLine, lid: lid, bid: blocks[bid].bid + 1}, jobId: job.id});
	...
	function processFirstLine(bid) {
		...
		if (targetBlock.forward && targetBlock.shuffleLastLine) {
			...
			if (targetBlock.shuffleTo != app.wid) {
				grid.send(app.worker[targetBlock.shuffleTo].uuid, {cmd: 'lastLine', args: {lastLine: shuffledLine, lid: lid, bid: bid + 1}});
