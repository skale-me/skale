...
function TextFileRDD(grid, app, job, param) {
	...
	function sendFirstLine(block, lastline, firstline) {
		var str;
		if ((lastline != undefined) || (block.lastline != undefined))
			str = ((lastline == undefined) ? '' : lastline) + ((firstline == undefined) ? '' : firstline);
		grid.send(app.worker[param.args[2][block.bid - 1].wid].uuid, {
			cmd: job.id + '.' + self.id + '.firstline', 
			data: {firstline: str, bid: (block.bid - 1)}
		});
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		grid.once(job.id + '.' + self.id + '.firstline', function (data) {
			...
			if (!block.lastline_reached) {
				block.lastline = firstline;
				return;
			}

			if ((block.lastline != undefined) || (firstline != undefined)) {
				var lines = ((block.lastline ? block.lastline : '') + (firstline ? firstline : '')).split(/\r\n|\r|\n/);
				if (block.firstline_sent) {
					for (var i = 0; i < lines.length; i++) block.processLine(lines[i]);
				} else {
					sendFirstLine(block, lines[0]);
					for (var i = 1; i < lines.length; i++) block.processLine(lines[i]);
				}
			}
			block.done();
		});
	...
	this.iterate = function(p, pipeline, done) {
		...
		var processDataOnce = function(chunk) {
			...
			if (lines.length) {																		// si il y a au moins 2 lignes
				...
				rs.on('data', processDataOther);													// passage au régime établi
			} else rs.once('data', processDataOnce);												// sinon on attend la complétion de la première ligne	
		}
		...
		rs.on('end', function() {
			...
			if (!block.lastline_received) {
				block.lastline = chunk_buffer ? chunk_buffer : undefined;
				return;
			}

			var lines = (chunk_buffer + (block.lastline ? block.lastline : '')).split(/\r\n|\r|\n/);
			if (block.firstline_sent) {
		...
		if (block.bid) rs.once('data', processDataOnce);
		else rs.on('data', processDataOther);
	}
}
// ---------------------------------------------------------------------------------------------------- //
