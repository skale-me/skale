...
function RDD(grid, app, job, param) {
	...
	var partition_dir = worker_dir + 'parts/';
	...
	this.shuffle = function() {
		...
		function fetchShuffleFile(from, done) {
			// if remote worker is on same host, no need to sftp shuffle file
			// self.postShuffleFiles[i] = '/tmp/ugrid/rdd_' + self.id + '/worker_' + i + '/shuffle/to_worker_' + app.wid;		// SAME HOST CASE
	...
	this.transform = function(context, data) {		
		...
		if (withReplacement) {
			for (var i = 0; i < data.length; i++) {
				var x = rng.sample();
				if (x == 0) continue;
				for (var j = 0; j < x; j++) tmp.push(data[i]);
			}
		} else {
			for (var i = 0; i < data.length; i++) {
				if (rng.nextDouble() >= frac) continue;
				tmp[i] = data[i];
			}			
		}
		return tmp;
