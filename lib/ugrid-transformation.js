...
module.exports.UgridJob = function(grid, app, param) {
	...
	for (var i = 0; i < param.node.length; i++) {
		if (app.da[param.node[i].id] == undefined) {
			...
			case 'cartesian': this.da[param.node[i].id] = new CartesianDA(grid, app, this, param.node[i]); break;
...
function DA(grid, app, job, param) {
	...
	this.pipeline = function(p, done, action) {
		...
		while ((sourcePartition.data == undefined) && sourcePartition.source) {
			if (sourceDA.persistent && !sourceDA.shuffling && !mm.storageFull()) {	// TO DEBUG partial storage RAM, add && (p == 0)
				sourcePartition.data = [];
				pipeline.unshift({transform: sourcePartition.save, p: sourcePartitionIdx});
			}
			if (sourceDA.transform) {
				var item = {transform: sourceDA.transform, p: sourcePartitionIdx};
				if (sourceDA.shuffling) item.sourceId = sourcePartition.source.id;
		...
		if (sourcePartition.data) sourcePartition.iterate(pipeline, done);
			...
			sourceDA.iterate(sourcePartitionIdx, pipeline, done);
		}
	}
...
function CartesianDA(grid, app, job, param) {
	ShuffledDA.call(this, grid, app, job, param);
	var self = this;
