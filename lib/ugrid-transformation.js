...
module.exports.UgridJob = function(uc, app, param) {
	this.id = param.jobId;
	this.sendResultSemaphore = 0;
	this.da = {};

	var stream = uc.createWriteStream(this.id, app.master_uuid), stageBoundaries = [], stages = [], self = this;

	// Step 1: Spawn DAs from top to bottom
	for (var i = 0; i < param.node.length; i++) {
		this.da[param.node[i].id] = (app.da[param.node[i].id]) ? 
			app.da[param.node[i].id] : 
			new lib[param.node[i].type](uc, app, this, param.node[i]);
	...
	this.sendResult = function(result) {
		...
		if (app.worker[app.wid + 1]) uc.send(app.worker[app.wid + 1].uuid, {cmd: 'action', jobId: self.id});
	}
...
function ShuffledDA(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
	var self = this;
	...
	this.run = function(nextStage) {
		...
		(function run() {
			if (++p < self.partitions.length) self.compute(p, function() {process.nextTick(run)});
				...
				self.preShuffle();
				for (var i = 0; i < app.worker.length; i++)
					if (uc.host.uuid == app.worker[i].uuid) self.shuffle();
					else uc.send(app.worker[i].uuid, {cmd: 'shuffle', jobId: job.id, daId: self.id});
			}
		})();
	}
	...
	this.shuffle = function() {
		...
		function fetchShuffleFile(from, done) {
			...
			if (uc.workerHost[from] == uc.workerHost[app.wid]) {
			...
			app.transfer(uc.workerHost[from], remote, local, function (err, res) {
...
// Sources
// ================================================================================================ //
function StreamDA(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
	...
	uc.on(streamIdx, onData);
	uc.on(streamIdx + '.block', onBlock);
	uc.on(streamIdx + '.end', onEnd);

	this.iterate = function(p, pipeline, done) {
		uc.send(app.master_uuid, {cmd: 'startStream', streamid: streamIdx, jobid: job.id});
		self.partitionDone = done;
		self.partitionPipeline = pipeline;
	};
};

function Parallelize(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
...
function RandomSVMData(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
...
function TextFile(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
...
function FlatMap(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
	var mapper = recompile(param.attr.mapper);
...
function MapValues(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
	var mapper = recompile(param.attr.mapper);
...
function FlatMapValues(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
	var mapper = recompile(param.attr.mapper);
...
function Filter(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
...
function Union(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
}

function Sample(uc, app, job, param) {
	DA.call(this, uc, app, job, param);
...
function Distinct(uc, app, job, param) {
	ShuffledDA.call(this, uc, app, job, param);
	var self = this;
...
function Cartesian(uc, app, job, param) {
	ShuffledDA.call(this, uc, app, job, param);
	var self = this;
...
function Intersect(uc, app, job, param) {
	ShuffledDA.call(this, uc, app, job, param);
	var self = this;
...
function Subtract(uc, app, job, param) {
	Intersect.call(this, uc, app, job, param);
	var self = this;
...
// ------------------------------------------------------------------------ //
function CombineByKey(uc, app, job, param) {
	ShuffledDA.call(this, uc, app, job, param);
	var self = this;
...
function ReduceByKey(uc, app, job, param) {
	CombineByKey.call(this, uc, app, job, param);
...
function GroupByKey(uc, app, job, param) {
	CombineByKey.call(this, uc, app, job, param);
...
function CoGroup(uc, app, job, param) {
	CombineByKey.call(this, uc, app, job, param);
	var self = this;
...
function Join(uc, app, job, param) {
	CoGroup.call(this, uc, app, job, param);
	var self = this;
...
function LeftOuterJoin(uc, app, job, param) {
	CoGroup.call(this, uc, app, job, param);
	var self = this;
...
function RightOuterJoin(uc, app, job, param) {
	CoGroup.call(this, uc, app, job, param);
	var self = this;
