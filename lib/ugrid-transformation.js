...
function RDD(grid, app, job, param) {
	...
	this.pipeline = function(p, done, action) {
		...
		while ((sourcePartition.data == undefined) && sourcePartition.source) {
			...
			if (sourceRDD.transform) pipeline.unshift({transform: sourceRDD.transform, p: sourcePartitionIdx});
...
function ReducedByKeyRDD(grid, app, job, param) {
	...
	this.transform = function(context, data) {
		for (var i = 0; i < data.length; i++) {
			...
			if (self.keys[key] == undefined) self.keys[key] = {buffer: ''};
			self.keys[key].buffer += value + '\n';
			self.totalLength += value.length;
			// buffer this key
			if (self.keys[key].buffer.length > self.MAX_BUFFER_SIZE) {
				fs.appendFileSync(self.tmp_dir + key, self.keys[key].buffer);
				self.totalLength -= self.keys[key].buffer.length;
				delete self.keys[key];
			}

			// flush all key if needed
			if (self.totalLength.length > (self.MAX_BUFFER_SIZE * 100)) {
				for (var k in self.keys) fs.appendFileSync(self.tmp_dir + k, self.keys[k].buffer);
				self.totalLength = 0;
				self.keys = {};
			}
		}
	...
	this.preShuffle = function() {
		for (var k in self.keys)
			fs.appendFileSync(self.tmp_dir + k, self.keys[k].buffer);
		// populate shuffle files
		...
		for (var i = 0; i < keys.length; i++) {
			...
			var data = JSON.parse('[' + content.replace(/\n/g, ',').slice(0, -1) + ']');
			var res = JSON.parse(JSON.stringify(param.args[0]));
			for (var v = 0; v < data.length; v++)
				res = reducer(res, data[v]);
			this.wFile[wid].buffer += JSON.stringify([JSON.parse(keys[i]), res]) + '\n';
	...
	this.iterate = function(p, pipeline, done) {
		...
		function populate(key_str, done) {
			...
			var acc = JSON.parse(JSON.stringify(param.args[0]));
			lines.on('data', function(line) {
				acc = reducer(acc, JSON.parse(line));
			});
			lines.on('end', function() {				
				var buffer = [[key, acc]];
				for (var t = 0; t < pipeline.length; t++)
...
function SampledRDD(grid, app, job, param) {
	...
	// collecter les partitions sur disque tout en déterminant leur nombre d'élément
	// ici il faut connaitre l'indice de la partition que l'on construit
	var partitions = [];
	this.transform = function(context, data) {
	...
	this.iterate = function(p, pipeline, done) {
		...
		// build index of entry to sample
