...
module.exports.UgridJob = function(grid, app, param) {
	...
	for (var i = 0; i < param.node.length; i++) {
		...
		if (this.node[param.node[i].num].dependency == 'wide')
			stageNodeNums.push(param.node[i].num);
	}
	...
	this.link = function(from, n) {
		...
		for (var i = 0; i < n.child.length; i++) this.link(n, n.child[i]);
	}
	this.link(null, this.node[root]);

	// Set number of lineage to be waited for by each sync node
	...
	// 	console.log('\nNode #' + this.node[i].num);
	// 	console.log('	type: ' + this.node[i].type);
	// 	console.log('	next: ' + this.node[i].next.map(function(n) {return n.num}));
	// 	console.log('	targetPathCnt: ' + this.node[i].nextTargetPathCount)		
	// }

	// console.log('\n------------------------------------------------------------------------')
	// console.log('digraph job {')
	// console.log('action');
	// for (var i in this.node) {
	// 	console.log(this.node[i].num + ' [label=' + this.node[i].type + '];');
	// 	for (var j = 0; j < this.node[i].next.length; j++) {
	// 		console.log(this.node[i].num + ' -> ' + (this.node[i].next[j].num || 'action'))
	// 	}
	// }
	// console.log('}');
...
function Transform(grid, app, job, node) {
	...
	this.exec = function() {
		...
		else for (var i = 0; i < this.child.length; i++) this.child[i].exec();
	}
	...
	// 		if (++this.pathCount < this.targetPathCount) return;			
	// 		if (this.type == 'join')
	// 			console.log(' <============================================================ JOIN shuffle')			
	// 		this.tx_shuffle();
	// 		if (this.nShuffle == app.worker.length) this.runFromStageRam();
	// 	}
	// }

	////// ATTENTION CI-DESSOU IL FAUT FIXER la gestion du nombre de targetPathCount
