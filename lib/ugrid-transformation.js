...
module.exports.UgridJob = function(grid, app, param) {
	...
	this.sendResult = function(result) {
		...
		// if collect notify master, else stream to master
		if (param.action.fun == 'collect') grid.send(app.master_uuid, {cmd: 'collect', jobId: self.id, path: self.result});
		else stream.write(self.result);
