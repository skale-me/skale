...
function Action(grid, app, job, action) {
	...
	this.sendResult = function() {
		...
		// reset stages
		for (i = 0; i < job.stage.length; i++) {
			job.stage[i].nShuffle = 0;
			job.stage[i].target_cnt = job.stage[i].next_target_cnt;
			job.stage[i].cnt = 0;
		}

		// set next iteration starting stage
		for (i = 0; i < job.stage.length; i++)
			if (job.stage[i].next_target_cnt !== 0) {
				job.scnt = i;
				break;
			}
...
function Transform(grid, app, job, node) {
	...
	this.tx_shuffle = function() {
		for (var i = 0; i < this.map.length; i++)
			...
			else grid.send(app.worker[i].uuid, {cmd: 'shuffle', args: this.map[i], jobId: job.id, sid: this.sid - 1});
	};
	...
	this.runFromRAM = function(callback) {
		job.stage[self.sid].locked = true;

		function readFile(path, p, end, cbk) {
			function finished() {
				if (end) {
					job.stage[self.sid].locked = false;
					job.stage[self.sid].next_target_cnt--;
				}
				cbk();
			}
	...
	this.runFromStageRam = function (callback) {
		var input = this.SRAM || [];
	...
	this.runSource = function(callback) {
		if (this.inMemory) this.runFromRAM(callback);
		else if (this.dependency == 'wide') this.runFromStageRam(callback);
		else this.run(callback);
	}

	////////////////////////////////////////////////////////////////////////////////////
	// GESTION DES STAGES DIRECTEMENT DANS LES NOEUDS
	////////////////////////////////////////////////////////////////////////////////////
	this.stage = {
		cnt: 0,									// Number of finished lineages
		target_cnt: this.nchild,				// Number of lineages to be waited for during this iteration
		...
		next_target_cnt: this.nchild			// next iteration lineage target count
	}
	////////////////////////////////////////////////////////////////////////////////////
	// GESTION DES STAGES DIRECTEMENT DANS LES NOEUDS
	////////////////////////////////////////////////////////////////////////////////////
...
module.exports.parallelize = function(grid, app, job, node) {
	...
	this.run = function(callback) {
		...
		job.stage[node.sid].next_target_cnt--;
		callback();
	};
}
...
module.exports.textFile = function(grid, app, job, node) {
	...
	function processBlock(bid) {
		...
		job.stage[node.sid].locked = true;
		...
		function shuffleLine(shuffledLine) {
			...
			if (shuffleTo != app.wid) {
				grid.send(app.worker[shuffleTo].uuid, {
					cmd: 'lastLine', 
					args: {lastLine: shuffledLine, sid: job.stage[node.sid].sid, lid: node.lid, bid: blocks[bid].bid + 1}, 
					jobId: job.id
				});
		...
		lines.on("endNewline", function(lastLineComplete) {
			...
			if ((blockIdx + 1) < blocks.length) {
				...
				job.stage[node.sid].locked = false;
				job.stage[node.sid].next_target_cnt--;
				cbk();
			}
		});
		...
		if (targetBlock.forward && targetBlock.shuffleLastLine) {
			...
			if (targetBlock.shuffleTo != app.wid) {
				grid.send(app.worker[targetBlock.shuffleTo].uuid, {
					cmd: 'lastLine', 
					args: {lastLine: shuffledLine, sid: job.stage[node.sid].sid, lid: node.lid, bid: bid + 1}
				});
...
module.exports.distinct = function(grid, app, job, node) {
	...
	this.rx_shuffle = function (data) {
		...
		if (++job.stage[node.sid - 1].nShuffle < app.worker.length) return;
