...
module.exports.UgridJob = function(grid, app, param) {
	...
	this.setSemaphore = function(n) {
		if (n.inMemory || (n.dependency == 'wide') || (n.child.length == 0)) {
			var stageNode = (n.stageNode == undefined) ? this.action : this.node[n.stageNode];
		...
		for (var i = 0; i < n.child.length; i++) this.setSemaphore(n.child[i]);
	}
	this.setSemaphore(this.node[root]);
	for (var i = 0; i < stageNodeNums.length; i++)
		for (var j = 0; j < this.node[stageNodeNums[i]].child.length; j++)
			this.setSemaphore(this.node[stageNodeNums[i]].child[j]);
