...
function DistinctRDD(grid, app, job, param) {
	...
	for (var to = 0; to < app.worker.length; to++) {
		file[to] = dir + 'distinct_' + wid + '_' + to;
		try {fs.unlinkSync(file[to]);} catch (e){;}			// DEBUG
		fs.appendFileSync(file[to], '');
	}

	this.transform = function(data) {
		for (var i = 0; i < data.length; i++) {
			var str = JSON.stringify(data[i]);
			fs.appendFileSync(file[ml.cksum(str) % app.worker.length], str + '\n');
		}
	};
