...
function ShuffledRDD(grid, app, job, param) {
	...
	this.postShuffle = function() {
		...
		function populate(from, to, done) {
			...
			lines.on('data', self.onPostShuffleLine);
			lines.on('end', function() {
				if (++from < app.worker.length) populate(from, to, done);
					...
					self.spillToDisk('postshuffle', self.intermediateFiles);
					done();
				}
			});
		}
...
function DistinctRDD(grid, app, job, param) {
	...
	// 	// entrée de buffer: {str: [cksum, value]}
	// 	// step 1: Prepare file buffers, flush global buffer, reset shuffleMemory
	// 	if (type == 'preshuffle') {
	// 		for (var key in self.buffer) {
	// 			var wid = self.buffer[key][0] % files.length;							// cksum doit etre toujours au meme endroit
	// 			files[wid].buffer += JSON.stringify([self.buffer[key], key]) + '\n';
	// 		}
	// 	} else if (type == 'postshuffle') {
	// 		for (var key in self.buffer) {
	// 			var fid = self.buffer[key][0] % files.length;
	// 			files[fid].buffer += JSON.stringify([key, self.buffer[key][1]]) + '\n';
	// 		}
	// 	}
	// 	// self.buffer = {};
	// 	// mm.shuffleMemory = 0;
	...
	this.onPostShuffleLine = function(line) {
...
function IntersectedRDD(grid, app, job, param) {
	...
	this.spillToDisk = function(type, files) {
		...
		if (type == 'preshuffle') {
			for (var key in self.buffer) {
				var wid = self.buffer[key][0] % files.length;
				files[wid].buffer += JSON.stringify([self.buffer[key], key]) + '\n';
			}
		} else if (type == 'postshuffle') {
			for (var key in self.buffer) {
	...
	this.transform = function(context, data) {
		...
		for (var i = 0; i < data.length; i++) {
			var str = JSON.stringify(data[i]);
			if (self.buffer[str] == undefined) {
				self.buffer[str] = [ml.cksum(str), [false, false]];
				mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);		// estimate shuffle memory size
			}
			self.buffer[str][1][sid] = true;			
			if (mm.shuffleFull()) {
				console.log('Spilling data to shuffle files during transform')
				self.spillToDisk('preshuffle', self.preShuffleFiles);
			}
	...
	this.onPostShuffleLine = function(line) {
		var raw = JSON.parse(line);	// data = [cksum, entry]
		var cksum = raw[0][0], value = raw[0][1], str = raw[1];
		////////////////////////////////////////////////
		// combining data here
		if (self.buffer[str] == undefined) {
			self.buffer[str] = [cksum, [false, false]];
			mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);
		} 
	...
	this.iterate = function(p, pipeline, done) {
		...
		try {
			...
			lines.on('data', function(line) {
				...
				var str = data[0], value = data[1];
				// combining data here
				if (self.buffer[str] == undefined) self.buffer[str] = [false, false];
				self.buffer[str][0] = self.buffer[str][0] || value[0];
				self.buffer[str][1] = self.buffer[str][1] || value[1];
			});
			lines.on('end', function() {
				for (var str in self.buffer) {
					if (self.buffer[str][0] && self.buffer[str][1]) {
						var buffer = [JSON.parse(str)];
						for (var t = 0; t < pipeline.length; t++)
							buffer = pipeline[t].transform(pipeline[t], buffer);						
					}
				}
...
function CombinedByKeyRDD(grid, app, job, param) {
	...
	// Spill to disk spécifique au PairRDD
	this.spillToDisk = function(type, files) {
		// step 1: Prepare file buffers, flush global buffer, reset shuffleMemory
		if (type == 'preshuffle') {
			for (var key in self.buffer) {
			...
			for (var key in self.buffer) {
				var fid = self.buffer[key][0] % files.length;
				files[fid].buffer += '[' + JSON.stringify(key) + ', ' + JSON.stringify(self.buffer[key][1]) + ']\n';
			}
		}
		self.buffer = {};
		mm.shuffleMemory = 0;
	...
	this.transform = function(context, data) {
		for (var i = 0; i < data.length; i++) {
			...
			if (self.buffer[str] == undefined) self.buffer[str] = [ml.cksum(str), JSON.parse(JSON.stringify(self.init))];
			mm.shuffleMemory -= sizeOf(self.buffer[str][1]);
			self.buffer[str][1] = self.reducer(self.buffer[str][1], value);
			mm.shuffleMemory += sizeOf(self.buffer[str][1]);
			if (mm.shuffleFull()) {
				console.log('Spilling data to shuffle files during transform')
				self.spillToDisk('preshuffle', self.preShuffleFiles);
	...
	this.onPostShuffleLine = function(line) {
		...
		var cksum = raw[0][0], value = raw[0][1], str = raw[1];
		if (self.buffer[str] == undefined) self.buffer[str] = [cksum, value];
		else {
			mm.shuffleMemory -= sizeOf(self.buffer[str][1]);
			self.buffer[str][1] = self.combiner(self.buffer[str][1], value);
			mm.shuffleMemory += sizeOf(self.buffer[str][1]);
		}
		if (mm.shuffleFull()) {
			console.log('Spilling data to intermediate files during postShuffle')
...
function CoGroupedRDD(grid, app, job, param) {
	...
	this.transform = function(context, data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (self.buffer[str] == undefined)
				self.buffer[str] = [ml.cksum(str), [[], []]];
			self.buffer[str][1][sid].push(value);
