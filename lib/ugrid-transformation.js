...
module.exports.groupByKey = function(grid, app, job, stage, node) {		// REWRITE
	Transform.call(this, grid, app, job, stage, node);
	this.result = {};
	var SRAM = this.SRAM = [];
	...
	this.tx_shuffle = function() {
		var map = app.worker.map(function() {return {};});
		for (var p in this.result)
			map[ml.cksum(p) % app.worker.length][p] = this.result[p];
		for (var i = 0; i < map.length; i++)
			if (grid.host.uuid == app.worker[i].uuid) this.rx_shuffle(map[i], stage);
			else grid.send(app.worker[i].uuid, {cmd: 'shuffle', args: map[i], jobId: job.id, sid: stage.sid});
	};
...
module.exports.reduceByKey = function(grid, app, job, stage, node) {	// REWRITE
	Transform.call(this, grid, app, job, stage, node);
	this.result = {};
	...
	this.tx_shuffle = function() {
		for (var key in this.result)
			map[ml.cksum(key) % app.worker.length][key] = this.result[key];
		for (var i = 0; i < map.length; i++)
			if (grid.host.uuid == app.worker[i].uuid) this.rx_shuffle(map[i], stage);
			else grid.send(app.worker[i].uuid, {cmd: 'shuffle', args: map[i], jobId: job.id, sid: stage.sid});
	};
...
module.exports.coGroup = function(grid, app, job, stage, node) {		// REWRITE
	Transform.call(this, grid, app, job, stage, node);
	this.result = {};
	var SRAM = this.SRAM = {};
...
module.exports.crossProduct = function(grid, app, job, stage, node) {
	...
	var map = app.worker.map(function() {return shuffledPartitions;});
	var rxPartitions = [];
...
module.exports.sortByKey = function(grid, app, job, stage, node) {
	...
	this.SRAM = [];
	var tmp = {}, tmp2 = {}, keys = [];
	var map = app.worker.map(function() {return tmp;});
