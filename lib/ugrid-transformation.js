...
function StreamRDD(grid, app, job, param) {
	RDD.call(this, grid, app, job, param);
 	...
 	var n = 0, N = param.args[0], streamIdx = param.args[1];
	...
	this.partitionDone; this.partitionPipeline; this.running = false; this.buffer; this.dataDone;	// to be flushed

	var onData = function(data, done) {
		if (this.running = false) {	// bufferize data and done
			self.buffer = data;
			self.dataDone = done;
			return;
		} else {	// process data normally
			var buffer = [data];
	...
	this.iterate = function(p, pipeline, done) {
		...
		if (self.buffer) {
			...
			for (var t = 0; t < self.partitionPipeline.length; t++)
				buffer = self.partitionPipeline[t].transform(pipeline[t], buffer);
