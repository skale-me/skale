...
module.exports.UgridJob = function(grid, app, param) {
	...
	for (var i = 0; i < param.node.length; i++) {
		if (app.rdd[param.node[i].id] == undefined) {
			...
			case 'intersection': this.rdd[param.node[i].id] = new IntersectionedRDD(grid, app, this, param.node[i]); break;
...
/* ---------------------------------------------------------------------------------------------------- //
Opération de Shuffle :
	1 - lors de la transformation les données sont processées et accumulées dans la RAM
	2 - quand le quota de données dans la RAM est atteint on les écrit dans les fichier de shuffle
	3 - lors de l'étape de preShuffle on flush le reste des données dans les fichiers de shuffle
	4 - lors du shuffle les workers s'informe mutuellement de la disponibilité des fichiers de shuffle
	5 - Les fichiers de shuffle sont rapatriés par chaque worker
	6 - Lors de la phase de postShuffle les fichiers de shuffle sont traités de sorte que l'on crée
	    sur chaque worker un fichier par partition
	7 - Lors du stage suivant l'iterateur de chaque partition prend en entrée son fichier de partition
	    correspondant

Transformations:
	- distinct (suppression des doublons cross partition)
	- intersect (garde les entrées dans les 2 datasets)
	- subtract (suprresion des entrées du second dataset)
	- cartesian 
	- combinedByKey
	- toutes les autres transformations dérivent des précédentes, seul l'itérateur diffère
// ---------------------------------------------------------------------------------------------------- */
function ShuffledRDD(grid, app, job, param) {
	...
	// Create preShuffle files structure (one per worker)
	for (var to = 0; to < app.worker.length; to++) {
		this.preShuffleFiles[to] = {
			name: self.shuffle_dir + 'to_worker_' + to,
			buffer: ''
		}
		try {fs.unlinkSync(this.preShuffleFiles[to].name);} catch (e){;}			// DEBUG
		fs.appendFileSync(this.preShuffleFiles[to].name, '');		
	}

	this.shuffle = function() {
		if (++this.nShuffle < app.worker.length) return;
		// SSH Connexions must be kept opened during context lifespan to eliminate connexion overhead
		function fetchShuffleFile(from, done) {
			...
			var sameHost = true;
			if (sameHost) { 	// if remote worker is on same host, no need to sftp shuffle file
			...
			conn.on('ready', function() {
				...
				host: 'localhost',					// connexion hostname must be known to open ssh connexion
...
function DistinctRDD(grid, app, job, param) {
	...
	function spillToDisk(type, files) {
		// step 1: Prepare file buffers, flush global buffer, reset shuffleMemory
		if (type == 'preshuffle') {
			for (var key in self.buffer) {
				var wid = self.buffer[key] % files.length;
				files[wid].buffer += '[' + self.buffer[key] + ',"' + key + '"]\n';
			}
		} else if (type == 'postshuffle') {
			for (var key in self.buffer) {
				var fid = self.buffer[key] % files.length;
				files[fid].buffer += key + '\n';
			}			
		}
		self.buffer = {};
		mm.shuffleMemory = 0;
	...
	this.transform = function(context, data) {
		for (var i = 0; i < data.length; i++) {
			...
			if (self.buffer[str] == undefined) {
				self.buffer[str] = ml.cksum(str);
				mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);		// estimate shuffle memory size
			}
			if (mm.shuffleFull()) {
				console.log('Spilling data to shuffle files during transform')
				spillToDisk('preshuffle', self.preShuffleFiles);
			}
		}
	};

	this.preShuffle = function() {
		console.log('Spilling data to shuffle files during preShuffle');
		spillToDisk('preshuffle', self.preShuffleFiles);
	}
	...
	this.postShuffle = function() {
		...
		for (var p = 0; p < self.P; p++) {
			self.intermediateFiles[p] = {
				name: self.intermediate_dir + 'p_' + app.wid + '_' + p,
				buffer: ''
			}
			try {fs.unlinkSync(self.intermediateFiles[p].name);} catch (e){;}			// DEBUG
		...
		function populate(from, to, done) {
			...
			lines.on('data', function(line) {
				var raw = JSON.parse(line);	// data = [cksum, entry]
				var cksum = raw[0], str = raw[1];
				if (self.buffer[str] == undefined) {
					self.buffer[str] = cksum;
					mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);
				}
				if (mm.shuffleFull()) {
					console.log('Spilling data to intermediate files during postShuffle')
					spillToDisk('postshuffle', self.intermediateFiles);
				}
			});
			lines.on('end', function() {
				if (++from < app.worker.length) populate(from, to, done);
				else {
					console.log('Spilling data to intermediate files at the end of postShuffle')
					spillToDisk('postshuffle', self.intermediateFiles);
					done();
				}
			});
...
function IntersectionedRDD(grid, app, job, param) {
	ShuffledRDD.call(this, grid, app, job, param);
	var self = this;
	...
	this.transform = function(context, data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			self.preShuffleFiles[wid].buffer += '[' + sid + ', ' + str + ']\n';
			if (self.preShuffleFiles[wid].buffer.length > self.MAX_BUFFER_SIZE) {
				fs.appendFileSync(self.preShuffleFiles[wid].name, self.preShuffleFiles[wid].buffer);
				self.preShuffleFiles[wid].buffer = '';
			}
		}
	};
	...
	this.preShuffle = function() {
		for (var i = 0; i < app.worker.length; i++)
			if (this.preShuffleFiles[i].buffer.length)
				fs.appendFileSync(this.preShuffleFiles[i].name, this.preShuffleFiles[i].buffer);
	}

	this.postShuffle = function() {
		// create intermediate files for iterator (one per partition)
		this.pFile = {};
		for (var p = 0; p < this.partitions.length; p++) {
			...
			try {fs.unlinkSync(self.pFile[p].name);} catch (e){;}		// DEBUG
			fs.appendFileSync(self.pFile[p].name, '');
		}
		// Population des fichiers de partitions à partir des fichiers de shuffles
		function populate(from, to, done) {
			var lines = new Lines();
			fs.createReadStream(self.postShuffleFiles[from]).pipe(lines);
			lines.on('data', function(line) {
	...
	this.iterate = function(p, pipeline, done) {
		...
		try {
			...
			lines.on('data', function(line) {
				...
				var sid = data[0];
				var sub = JSON.stringify(data[1]);
				for (var idx = 0; idx < index.length; idx++)
					if (index[idx][0] == sub) break;
				if (index[idx] == undefined) index[idx] = [sub, false, false];
				index[idx][sid + 1] = true;
			});
			lines.on('end', function() {
				for (var i = 0; i < index.length; i++) {
					if (index[i][1] && index[i][2]) {
						var buffer = [JSON.parse(index[i][0])];
						for (var t = 0; t < pipeline.length; t++)
							buffer = pipeline[t].transform(pipeline[t], buffer);						
					}
				}
...
function CombinedByKeyRDD(grid, app, job, param) {
	...
	this.transform = function(context, data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			// part[idx][1] = self.reducer(part[idx][1], value);	// old version
	...
	this.preShuffle = function() {	// A APPELER A LA FIN DE CHAQUE PARTITION ????
		...
		// mm.report();
		console.log('Reached preshuffle');
		var combined = [];
		...
		// step 2: consolidate data in shuffle files (one per worker)
		/*
			ICI il faut utiliser au maximum la RAM allouée au shuffle afin d'éviter
			les appels trop fréquents à appendFileSync
		*/
		for (var i = 0; i < combined.length; i++) {
			var key = combined[i][0], value = combined[i][1];
			var wid = ml.cksum(key) % app.worker.length;
			this.preShuffleFiles[wid].buffer += JSON.stringify(combined[i]) + '\n';
			if (this.preShuffleFiles[wid].buffer.length > this.MAX_BUFFER_SIZE) {
				fs.appendFileSync(this.preShuffleFiles[wid].name, this.preShuffleFiles[wid].buffer);
				this.preShuffleFiles[wid].buffer = '';
			}
		}
	...
	this.postShuffle = function() {
		function populate(from, done) {
			// Ici il faut utiliser la RAM afin d'éviter d'écrire un fichier par partition
			var rs = fs.createReadStream(self.postShuffleFiles[from]), chunk_buffer = '';
	...
	this.iterate = function(p, pipeline, done) {
		...
		function populate(key_str, done) {
			...
			var rs = fs.createReadStream(self.intermediate_dir + key_str), chunk_buffer = '';
			var acc = JSON.parse(JSON.stringify(self.init));
			rs.on('data', function (chunk) {
				var lines = (chunk_buffer + chunk).split(/\r\n|\r|\n/);
				chunk_buffer = lines.pop();
				for (var i = 0; i < lines.length; ++i)
					acc = self.combiner(acc, JSON.parse(lines[i]));
			})
			rs.on('end', function() {
				var buffer = [[key, acc]];
				for (var t = 0; t < pipeline.length; t++)
...
// ------------------------------------------------------------------------------------ //
// Transformation (crossProduct, partitionByKey, sortByKey)
// ------------------------------------------------------------------------------------ //
