...
module.exports.UgridJob = function(grid, app, param) {
	...
	var streamcollect = grid.createWriteStream('stream.' + this.id, app.master_uuid)	
	...
	this.sendResult = function(result) {
		...
		// If action is collect, stream each element one at a time else send result as one block of data
		// if (param.action.fun == 'collect') {
		// 	var i = 0;
		// 	streamcollect.write(self.result[i], done);
		// 	function done() {
		// 		if (i++ < self.result.length) streamcollect.write(self.result[i], done);
		// 		else end();
		// 	}
		if (param.action.fun == 'collect') {
			// plutot que de streamer le résultat, il faut notifier le master qu'il peut 
			// récupérer le fichier de résultat partiel
			var lines = new Lines();
			fs.createReadStream(self.result).pipe(lines);
			lines.pipe(streamcollect);
			lines.on('end', end);
		} else {
			...
			// Notify master that job is finished
...
function RDD(grid, app, job, param) {
	...
	this.spillCollectToDisk = function(data) {
		var buffer = '';
		for (var i = 0; i < data.length; i++)
			buffer += JSON.stringify(data) + '\n';
		data = [];
		mm.collectMemory = 0;
		fs.appendFileSync(this.worker_dir + 'collect', buffer);
	}
			...
			if (++p < self.partitions.length) {
				self.pipeline(p, function processPartition() {process.nextTick(action)},
					function processElement(context, data) {
						...
						result += str;
						if (mm.collectFull()) {
							fs.appendFileSync(self.worker_dir + 'collect', result);
							mm.collectMemory = 0;
							result = '';
						}
					}
				...
				fs.appendFileSync(self.worker_dir + 'collect', result); // flush memory
				mm.collectMemory = 0;
				result = '';
				callback(self.worker_dir + 'collect');
			}
		})();
	};
	...
	// 				},
	// 				function processElement(context, data) {
	// 					// Ici il faut ecrire sur disque si jamais le collect explose en memoire
	// 					// dans le sendResult il faut lire le fichier de résultat si il existe
	// 					// et terminer par le résidu de la RAM
	// 					mm.collectMemory += sizeOf(data);
	// 					if (mm.collectFull()) {
	// 						// console.log('Collect Memory is full, need to spill to disk')
	// 						// self.spillToDisk('preshuffle', self.preShuffleFiles);
	// 						// self.spillCollectToDisk(data);
	// 					} else {
	// 						// console.log('Still some space')
...
function ShuffledRDD(grid, app, job, param) {
	...
	var partition_dir = this.worker_dir + 'parts/';	// Will be used when persisting on disk
