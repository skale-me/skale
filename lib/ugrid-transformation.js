...
function Action(grid, app, job, action) {
	...
	this.semaphore = 0;
	this.pathCount = 0;
	this.targetPathCount = action.target_cnt;
...
function Transform(grid, app, job, node) {
	for (var key in node) this[key] = node[key];	// May use ".bind()" instead of "new" operator
	var ram = {};
	...
	this.targetPathCount = this.target_cnt;
	...
	// Replace child id and child anc id by references
	for (var i = 0; i < this.child.length; i++) {
		job.node[this.child[i]].anc = this;
	...
	this.pipelineToAction = function(p, head) {
		for (var t = 1; t < this.nodePath.length; t++) {
			this.tmp = this.nodePath[t].pipeline(this.tmp, p, this.nodePath[t - 1].id);
			if (this.tmp && (this.tmp.length === 0)) return;
			if (this.nodePath[t].persistent && (this.nodePath[t].dependency == 'narrow'))
				this.nodePath[t].save(this.tmp, head);
		}
		if (this.inLastStage)
	...
	this.runFromStageRam = function () {
		...
		for (var p = 0; p < input.length; p++) {
			...
			for (var i = 0; i < partition.length; i++) {
				...
				if (this.persistent) this.save(this.tmp);
...
module.exports.stream = function(grid, app, job, node) {
	...
	var onData = function(data, done) {			
		self.tmp = [data];
		if (self.persistent) self.save(self.tmp);
		self.pipelineToAction(0);
		if (++n == N) {
			n = 0;
...
module.exports.sample = function(grid, app, job, node) {
	...
	this.tx_shuffle = function() {
		...
		this.nShuffle = app.worker.length;	// Cancel shuffle
	};
};
