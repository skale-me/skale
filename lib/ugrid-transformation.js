...
function GroupedByKeyRDD(grid, app, job, param) {
	...
	this.postShuffle = function() {
		function populate(from, to, done) {
			var lines = new Lines();
			fs.createReadStream(self.base_dir + 'w_' + from + '_' + to).pipe(lines);
			lines.on('data', function(line) {
	...
	this.iterate = function(p, pipeline, done) {
		// on récupère la liste des fichiers dans partitions dir
		...
		function populate(key_str, done) {
			...
			lines.on('data', function(line) {
				var buffer = [key, JSON.parse(line)];
				for (var t = 0; t < pipeline.length; t++)
					buffer = pipeline[t](buffer);
			});
			lines.on('end', function() {
...
function DistinctRDD(grid, app, job, param) {
	...
	this.shuffling = true;
	var file = {}, pFile = {}, MAX_BUFFER_SIZE = 8192;
	var P = 100;	// number of partitions
	var dir = '/tmp/' + this.id + '/';

	// Create shuffle directory and shuffle files
	try{fs.mkdirSync(dir)} catch (e) {};
	for (var to = 0; to < app.worker.length; to++) {
		file[to] = {
			name: dir + 'w_' + app.wid + '_' + to,
			buffer: ''
		}
		try {fs.unlinkSync(file[to].name);} catch (e){;}			// DEBUG
		fs.appendFileSync(file[to].name, '');
	}

	this.preShuffle = function() {
		for (var i = 0; i < app.worker.length; i++)
			if (file[i].buffer.length) {
				fs.appendFileSync(file[i].name, file[i].buffer);
				file[i].buffer = '';
			}
	}
	...
	this.transform = function(data) {
		for (var i = 0; i < data.length; i++) {
			...
			file[wid].buffer += str + '\n';
			if (file[wid].buffer.length > MAX_BUFFER_SIZE) {
				fs.appendFileSync(file[wid].name, file[wid].buffer);
				file[wid].buffer = '';
			}
		}
	};
	...
	this.postShuffle = function() {
		...
		function populate(from, to, done) {
			var lines = new Lines();
			fs.createReadStream(dir + 'w_' + from + '_' + to).pipe(lines);
			lines.on('data', function(line) {
				var fid = ml.cksum(line) % P;
				pFile[fid].buffer += line + '\n';
				if (pFile[fid].buffer.length > MAX_BUFFER_SIZE) {
					fs.appendFileSync(pFile[fid].name, pFile[fid].buffer);
					pFile[fid].buffer = '';
				}
			});
			lines.on('end', function() {
				if (++from < app.worker.length) populate(from, to, done);
					...
					for (var fid = 0; fid < P; fid++)
						if (pFile[fid].buffer.length)
							fs.appendFileSync(pFile[fid].name, pFile[fid].buffer);
					done();
				}
			});
	...
	this.shuffle = function (data) {
		...
		for (var p = 0; p < P; p++) {
			this.partitions.push(new Partition(null));
			pFile[p] = {
				name: dir + 'p_' + app.wid + '_' + p,
				buffer: ''
			}
			try {fs.unlinkSync(pFile[p].name);} catch (e){;}		// DEBUG
			fs.appendFileSync(pFile[p].name, '');
		}
		this.postShuffle();
	};
	...
	this.iterate = function(p, pipeline, done) {
		...
		try {
			fs.createReadStream(pFile[p].name).pipe(lines);
			lines.on('data', function(line) {
