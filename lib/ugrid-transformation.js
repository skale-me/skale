...
function ShuffledRDD(grid, app, job, param) {
	...
	this.postShuffle = function() {
		...
		console.log('Reached postShuffle')
	...
	this.iterate = function(p, pipeline, done) {
		...
		try {
			...
			lines.on('data', function(line) {self.onIteratorLine(pipeline, line);});
...
function DistinctRDD(grid, app, job, param) {
	...
	this.onPostShuffleLine = function(line) {
		...
		var cksum = raw[0], data = raw[1][0], str = JSON.stringify(data);
		if (self.buffer[str] == undefined) {
			self.buffer[str] = [cksum, [data]];
			mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);
		}
	...
	this.onIteratorLine = function(pipeline, line) {
...
function IntersectedRDD(grid, app, job, param) {
	...
	this.onPostShuffleLine = function(line) {
		var raw = JSON.parse(line);
		var cksum = raw[0], data = raw[1][0], str = JSON.stringify(data), value = raw[1][1];
		if (self.buffer[str] == undefined) {
			self.buffer[str] = [cksum, [data, [false, false]]];
			mm.shuffleMemory += sizeOf(self.buffer[str]) + sizeOf(str);
		}
