...
function RDD(grid, app, job, param) {
	...
	// 			} else self.rx_shuffle();	// for SampledRDD, it does not shuffle any data
	// 		}
	// 	})();
	// }
	...
	this.run = function(nextStage) {
		...
		(function run() {
			if (++p < self.partitions.length) self.pipeline(p, function() {process.nextTick(run)});
				...
				self.flush();	// flush shuffle files buffers
				for (var i = 0; i < app.worker.length; i++)
					if (grid.host.uuid == app.worker[i].uuid) self.rx_shuffle();
...
function DistinctRDD(grid, app, job, param) {
	...
	this.shuffling = true;
	var file = {}, pFile = [], wid, MAX_BUFFER_SIZE = 8192;
	...
	// Look for my id
	for (var i = 0; i < app.worker.length; i++)
		if (app.worker[i].uuid == grid.host.uuid) {
			wid = i;
			break;
		}
	...
	this.rx_shuffle = function (data) {
		...
		// creation des partitions et des fichiers associés
		this.partitions = [];
		for (var p = 0; p < P; p++) {
			this.partitions.push(new Partition(null));
			pFile[p] = dir + 'pDistinct_' + wid + '_' + p;
			try {fs.unlinkSync(pFile[p]);} catch (e){;}		// DEBUG
			fs.appendFileSync(pFile[p], '');
		}
		// Population des fichiers de partitions à partir des fichiers de shuffles		
		function populate(from, to, done) {
			var lines = new Lines();
			fs.createReadStream(dir + 'distinct_' + from + '_' + to).pipe(lines);
			lines.on('data', function(line) {
				fs.appendFileSync(pFile[ml.cksum(line) % P], line + '\n');
			});
		...
		populate(0, wid, this.nextStage);
	};

	this.iterate = function(p, pipeline, done) {
		var lines = new Lines(), index = [];
		try {
			fs.createReadStream(pFile[p]).pipe(lines);
			lines.on('data', function(line) {
