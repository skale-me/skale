...
function TextFileRDD(grid, app, job, param) {
	...
	for (var i = 0; i < blocks.length; i++) {
		grid.once(job.id + '.' + self.id + '.firstline', function(data) {
			...
			if (block.processed) {
				if (block.firstline_sent) {
					var str;
					if ((firstline != undefined) || (block.lastline != undefined)) {
						var str = ((block.lastline == undefined) ? '' : block.lastline) + 
							((firstline == undefined) ? '' : firstline);			// on combine lastline avec firstline
					}
					if (str != undefined) {
						var lines = str.split(/\r\n|\r|\n/);
	...
	this.iterate = function(p, pipeline, done) {
		...
		var processDataOnce = function(chunk) {
			...
			if (lines.length) {															// SI il y a plus d'une ligne dans le block
				...
				rs.on('data', processDataOther);										// positionnement du régime établi
			} else {																	// SINON il n'y a qu'une seule ligne dans le block
				if (block.lastline_received) {											// SI la première ligne du block suivant a été reçue
					var str;
					if ((chunk_buffer != undefined) || (block.lastline != undefined)) {
						var str = ((chunk_buffer == undefined) ? '' : chunk_buffer) + 
							((block.lastline == undefined) ? '' : block.lastline);		// on combine lastline avec firstline
					}
					sendFirstLine(block, str);											// on envoie le resultat au block precedent						
				} else {																// SINON la premiere ligne du block suivant n'as pas encore été reçue
					block.lastline = chunk_buffer;										// on stocke la dernière ligne
					block.done = done;													// on stocke le done
					block.processLine = processLine;									// on stocke le processLine
				}
			}
		...
		rs.on('end', function() {
			...
			if (block.bid == param.args[2].length - 1) {								// SI c'est le dernier block
				if (block.firstline_sent == true) {										// si la premiere ligne a été envoyée
					if (chunk_buffer) processLine(chunk_buffer);						// on traite la dernière ligne
				} else sendFirstLine(block, chunk_buffer);								// sinon on envoie le residu comme premiere ligne
				done();																	// on termine le traitement du block
			} else {																	// SINON ce n'est pas le dernier block
				if (block.firstline_sent && block.lastline_received) {
					var lines = (chunk_buffer + block.lastline).split(/\r\n|\r|\n/);
					for (var i = 0; i < lines.length; ++i) processLine(lines[i]);
					done();
				}
				if (!block.lastline_received) {
					block.lastline = (chunk_buffer == '') ? undefined : chunk_buffer;
					block.done = done;
					block.processLine = processLine;
				}
				if (block.lastline_received && !block.firstline_sent) {
					var str;
					if ((chunk_buffer != undefined) || (block.lastline != undefined)) {
						var str = ((chunk_buffer == undefined) ? '' : chunk_buffer) + 
							((block.lastline == undefined) ? '' : block.lastline);			// on combine lastline avec firstline
		...
		if (block.bid != 0) rs.on('data', processDataOnce);
		else {
			chunk_buffer = '';
