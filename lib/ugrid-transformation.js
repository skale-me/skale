...
function Action(grid, app, job, action) {
	...
	this.sendResult = function() {
		...
		for (n in job.node) {			
			if (job.node[n].transform && job.node[n].transform.reset)
				job.node[n].transform.reset();
		}
...
module.exports.groupByKey = function(grid, app, job, stage, node) {
	...
	// pendant le pipeline ecrire le contenu de map directement dans un fichier
	// faire un fichier par worker
	this.pipeline = function(array) {
		for (var i = 0; i < array.length; i++) {
			var key = array[i][0];
