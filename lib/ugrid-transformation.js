...
function Action(grid, app, job, action) {
	...
	this.sendResult = function(ignore) {
		var i, n, s, jobFinished;

		if ((app.wid === 0) || (++this.semaphore == 2)) {
			if (!ignore) {
				stream.write(this.result);
				if ((app.worker[app.wid + 1]) && ((app.wid + 1) < action.maxWorker))
				...
				for (i = 0; i < job.stage.length; i++)
					if (job.stage[i].next_target_cnt !== 0) break;
				if (i < job.stage.length) job.scnt = i;
			...
			var sid = job.stage.length - 1;
			job.stage[sid].target_cnt = job.stage[sid].next_target_cnt;
			job.stage[sid].cnt = 0;
		}
	};
}
...
module.exports.forEach = function(grid, app, job, action) {
	...
	this.reset = function() {this.semaphore = 0;};
