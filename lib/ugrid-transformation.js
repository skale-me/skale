...
function TakeOrdered(grid, action) {
...
function Union(grid, node) {
	this.pipeline = function (array, p) {return array;}
}

function Map(grid, node) {
	var mapper = node.src;
	var args = node.args;
...
function Sample(grid, node, worker) {
...
function GroupByKey(grid, node, worker) {		// REWRITE
	this.result = {};
	var SRAM = this.SRAM = [];
	...
	this.tx_shuffle = function(state) {
		...
		for (var i = 0; i < map.length; i++)
			...
			else grid.request(worker[i], {cmd: 'shuffle', args: map[i]}, function(err) {if (err) throw err;});
	}
...
function ReduceByKey(grid, node, worker) {	// REWRITE
	this.result = {};
...
function Join(grid, node, worker) {
	this.SRAM = [];
...
function CoGroup(grid, node, worker) {		// Rewrite
	this.result = {};
	var SRAM = this.SRAM = {};
...
function CrossProduct(grid, node, worker) {					// REWRITE
	this.result = {};
	var SRAM = this.SRAM = {};
...
function Distinct(grid, node, worker) {
	this.SRAM = [];
...
function Intersection(grid, node, worker) {
	this.SRAM = [];
...
function Subtract(grid, node, worker) {
	this.SRAM = [];
...
function FlatMapValues(grid, node) {
	var mapper = node.src;
...
function PartitionByKey(grid, node, worker) {
	this.SRAM = [];
...
function SortByKey(grid, node, worker) {
	this.SRAM = [];
...
module.exports.partitionByKey = PartitionByKey;
module.exports.takeOrdered = TakeOrdered;
