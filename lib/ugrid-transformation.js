...
module.exports.UgridJob = function(grid, app, param) {
	...
	for (var i = 0; i < param.node.length; i++) {
		if (app.rdd[param.node[i].id] == undefined) {
			...
			case 'textFile2': this.rdd[param.node[i].id] = new TextFile2RDD(grid, app, this, param.node[i]); break;			
...
/* il faut nécessairement envoyer la première ligne au worker gérant le block précédent */
// Pour le moment un block par worker, le mapping est donc connu
function TextFileRDD(grid, app, job, param) {
	...
	console.log(blocks)
	/* NB
		- Positionner les event receiver des first line ici et pas dans le iterate
		- on doit soit shuffle la première ligne, soit la derniere:
	*/
	...
	this.iterate = function(p, pipeline, done) {
		...
		if (blocks[p].bid > 0) {
			grid.on(job.id + '.' + blocks[p].bid + '.firstLine', function(data) {
				console.log(data);
			});
		}
		...
		var processDataOnce = function(chunk) {	// Skip first line
			...
			// Send firsrt line of all blocks execpt the first one to previous worker
			if ((blocks[p].bid > 0) && (app.worker.length > 1)) {
				console.log('sending line to previous worker:' + lines[0])
				grid.send(app.worker[app.wid - 1].uuid, {cmd: job.id + '.' + (blocks[p].bid - 1) + '.firstLine', data: lines[0]});
			}
...
function TextFile2RDD(grid, app, job, param) {
	RDD.call(this, grid, app, job, param);
	...
	// Set first line of next block event receiver for each block 
	for (var i = 0; i < blocks.length; i++) {
		grid.once(job.id + '.' + self.id + '.firstline', function(data) {
			var firstline = data.data.firstline;
			var block = param.args[2][data.data.bid];
			if (++block.semaphore == 2) {
				if ((firstline != undefined) || (block.lastline != undefined)) {
					var tmp = ((block.lastline == undefined) ? '' : block.lastline) + ((firstline == undefined) ? '' : firstline);
					var lines = tmp.split(/\r\n|\r|\n/);
					for (var i = 0; i < lines.length; ++i) block.processLine(lines[i]);
				}
				block.done();
			} else {
				block.lastline = firstline;
			}
		});
	}
	...
	this.iterate = function(p, pipeline, done) {
		...
		// If not first block, send first line to worker that handles previous block
		var processDataOnce = function(chunk) {
			var lines = (chunk_buffer + chunk).split(/\r\n|\r|\n/);
			chunk_buffer = lines.pop();
			// Send firsrt line of all blocks execpt the first one to previous worker
			var uuid = app.worker[param.args[2][blocks[p].bid - 1].wid].uuid
			grid.send(uuid, {cmd: job.id + '.' + self.id + '.firstline', data: {firstline: lines[0], bid: (blocks[p].bid - 1)}});
			// process next lines normally
			for (var i = 1; i < lines.length; ++i) processLine(lines[i]);
			rs.removeListener('data', processDataOnce);
			rs.on('data', processDataOther);
		}
		...
		rs.on('data', (blocks[p].bid != 0) ? processDataOnce : processDataOther);

		rs.on('end', function() {
			if (blocks[p].bid == param.args[2].length - 1) {
				// si il reste de la data dans le buffer on process la dernière ligne et on quitte
				if (chunk_buffer) processLine(chunk_buffer);
				done();
			} else {
				// sinon, si la dernière ligne du block suivant a été reçue on la combine au chunk buffer s'il n'est pas vide
				if (++blocks[p].semaphore == 2) {
					if ((chunk_buffer != undefined) || (blocks[p].lastline != undefined)) {
						var tmp = ((chunk_buffer == undefined) ? '' : chunk_buffer) + ((blocks[p].lastline == undefined) ? '' : blocks[p].lastline);
						var lines = tmp.split(/\r\n|\r|\n/);
						for (var i = 0; i < lines.length; ++i) processLine(lines[i]);
					}
					done();
				} else {
					blocks[p].lastline = (chunk_buffer == '') ? undefined : chunk_buffer;
					blocks[p].done = done;
					blocks[p].processLine = processLine;
				}
			}
		});
	}
