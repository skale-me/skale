...
function Partition(source, data, key) {
	...
	// flow controlled version
	// this.save = function(array, done) {
	// 	for (var i = 0; i < array.length; i++)
	// 		self.data.push(array[i]);
	// 	done(array);
	// }

	// flow controlled write to disk
	...
	// Flow controlled version
	// this.iterate = function(pipeline, done) {
	// 	var i = 0, buffer, N = 10;	// Set N to 1 for hysteric call to process.nextTick
	// 	var nextTick1 = N, nextTick2 = N;
	// 	(function f1() {
	// 		if (i < self.data.length) {
	// 			buffer = [self.data[i++]];
	// 			var t = 0;
	// 			(function f2() {
	// 				if (t < pipeline.length) 
	// 					pipeline[t++](buffer, function(res) {
	// 						buffer = res;
	// 						if (--nextTick2 == 0) {
	// 							nextTick2 = N;
	// 							process.nextTick(f2);
...
function RandomSVMData(grid, app, job, param) {
	...
	// flow controlled version
	// this.iterate = function(p, pipeline, done) {
	// 	var i = 0, buffer, rng = new ml.Random(partitions[p].seed);
	// 	(function f1() {
	// 		if (i++ < partitions[p].n) {
	// 			buffer = [ml.randomSVMLine(rng, D)];
	// 			var t = 0;
...
function DistinctRDD(grid, app, job, param) {
	...
	this.rx_shuffle = function (data) {
		...
		this.partitions = [new Partition(null, tmp.map(JSON.parse))];
		this.nextStage();
	};
};
