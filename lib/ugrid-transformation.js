...
function GroupByKey(node, worker) {
	this.result = {};
	var SRAM = this.SRAM = [];
	...
	this.tx_shuffle = function(state, grid) {
...
function ReduceByKey(node, worker) {
	this.result = {};
		...
		for (var i = 0; i < key0.length; i++) {
			...
			for (var j = 0; j < tmp[node.child[0]][key0[i]].length; j++)
				for (var k = 0; k < tmp[node.child[1]][key0[i]].length; k++)
					data.push([key0[i], [tmp[node.child[0]][key0[i]][j], tmp[node.child[1]][key0[i]][k]]])
			this.SRAM.push({key: key0[i], data: data});
		}
	}
}

function CoGroup(node, worker) {
	this.result = {};
	var SRAM = this.SRAM = {};
...
function CrossProduct(node, worker) {
	this.result = {};
	var SRAM = this.SRAM = {};
...
function Distinct(node, worker) {
	...
	this.tx_shuffle = function(state, grid) {
		for (var i = 0; i < worker.length; i++) {
			if (worker[i].uuid == grid.host.uuid) this.rx_shuffle(this.result[i], state);
