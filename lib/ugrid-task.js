...
function mapHDFSBlocks(blocks, workers) {
	if (blocks.length < workers.length)
		throw 'Temprorary error: blocks.length must be higher worker.length'
	var mapping = {};
	...
	for (var i = 0; i < blocks.length; i++) {
		if (mapping[blocks[i].host] == undefined)
			throw 'block host ' + blocks[i].host + ' unreachable'
		// find shortest worker block list for block host
		var min_id = undefined;
		for (w in mapping[blocks[i].host]) {			
			if (min_id == undefined) {
				min_id = w;
				continue;
			}
			if (mapping[blocks[i].host][w].length < mapping[blocks[i].host][min_id].length)
				min_id = w;
		}
		// map block to min_id worker
		mapping[blocks[i].host][min_id].push(blocks[i]);
	}
	var out = {};
...
function preBuildTask(grid, worker, array, action, callback) {
	...
	// hdfs works only for one file in dag	
...
function buildTask(grid, worker, array, action, callback) {
	preBuildTask(grid, worker, array, action, function() {
		...
		require('fs').writeFileSync('/tmp/debug.js', taskCode);
		// throw 'break';
...
var taskTemplate = function(grid, fs, readline, ml, STAGE_RAM, RAM, msg) {
