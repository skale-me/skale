...
function buildTask2(worker, array, action, callback) {
		...
		conn.exec(fsck_cmd, function(err, stream) {
			...
			rl.on("line", function (line) {
				...
				blocks.push({
					...
					file: v[1].substr(0, v[1].lastIndexOf('_')),
					host: v[4].substr(0, v[4].lastIndexOf(':')).replace('[', '')
				});
			});
			rl.on('close', function () {
				conn.end();
				// Ici je possède tous les blocks
				console.log(worker)
				console.log(" ")
				hdfsTextFileNodes[0].args[0] = blocks;
				console.log(hdfsTextFileNodes[0].args)
...
function buildTask(worker, array, action, callback) {
	...
	// buildTask2(worker, array, action, function() {
	// 	;
	// });
