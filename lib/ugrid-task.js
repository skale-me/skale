	...
	treewalk(array, null, function (a) {
		...
		if (a.inMemory) {
			...
			if (a.transform == 'parallelize') {
				if (a.persistent)
					t1 += 'RAM[' + a.id + '] = {};' + 
						'for (var p = 0; p < node[' + a.num + '].args[1]; p++) RAM[' + a.id + '][p] = [];';
				t1 += 'var p = 0;';
				t1 += 'for (var i = 0; i < node[' + a.num + '].args[0].length; i++) {';
				t1 += 'tmp = node[' + a.num + '].args[0][i];'
				if (a.persistent)
					t1 += 'RAM[' + a.id + '][p].push(tmp);';
				t1 += '"PIPELINE_HERE"';
				t1 += 'p = (p == (node[' + a.num + '].args[1] - 1)) ? 0 : p + 1;'
				t1 += '}';
			} else if (a.transform == 'randomSVMData') {
				if (a.persistent)
					t1 += 'RAM[' + a.id + '] = {};' + 
						'for (var p = 0; p < node[' + a.num + '].args[3]; p++) RAM[' + a.id + '][p] = [];';
				t1 += 'var p = 0; var rng = new ml.Random(node[' + a.num + '].args[2]);';
				t1 += 'for (var i = 0; i < node[' + a.num + '].args[0]; i++) {'
				t1 += 'tmp = ml.randomSVMLine(rng, node[' + a.num + '].args[1]);'
				if (a.persistent)
					t1 += 'RAM[' + a.id + '][p].push(tmp);';
				t1 += '"PIPELINE_HERE"';
				t1 += 'p = (p == (node[' + a.num + '].args[3] - 1)) ? 0 : p + 1;'
				t1 += '}';
			} else if (a.transform == 'textFile') {
				// ICI il faut
				// stage[a.stageIdx].callbackOutput = ''
				if (a.persistent)
					t1 += 'RAM[' + a.id + '] = {};' + 
						'for (var p = 0; p < node[' + a.num + '].args[3]; p++) RAM[' + a.id + '][p] = [];';
				t1 += 'var p = 0;';
				t1 += 'var i = 0;';
				t1 += 'var rl = readline.createInterface({input: fs.createReadStream(node[' + a.num + '].args[0]), output: process.stdout, terminal: false});';
				t1 += 'rl.on("line", function (tmp) {';
				if (a.persistent)
					t1 += 'RAM[' + a.id + '][p].push(tmp);';
				t1 += 'console.log("line: " + tmp);';									// DEBUG ICI
				t1 += '"PIPELINE_HERE"';
				t1 += 'p = (p == (node[' + a.num + '].args[3] - 1)) ? 0 : p + 1;';
				t1 += 'i++';
				t1 += '});';
			...
			if (stage[a.stageIdx].lineages.length == 0) {
				var t1 = 'var input = STAGE_RAM; for (var p in input) {' + 
...
var taskTemplate = function(grid, fs, readline, ml, STAGE_RAM, RAM, node, action) {	
	...
	function runStage(idx) {
		...
		var res = stage[idx](function (res) {
			if (lastStage) {
				STAGE_RAM = [];
				finalCallback(res);
			} else {
