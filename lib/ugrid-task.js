...
function mapHDFSBlocks(blocks, workers) {
	...
	for (var i = 0; i < workers.length; i++) {
		// WORKAROUND ICI, l'ip des workers est polluée
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		for (var j = 0; j < blocks[i].host.length; j++) {
			//~ console.log('\nblocks[' + i + '].host[' + j + '] = ' + blocks[i].host[j])
			for (w in mapping[blocks[i].host[j]]) {			// boucle sur les worker associés à l'host en cours du block
				//~ console.log('w = ' + w)
				if (min_id == undefined) {
					min_id = w;
