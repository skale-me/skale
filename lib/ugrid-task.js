...
var taskTemplate = function(grid, fs, util, Lines, ml, STAGE_RAM, RAM, msg) {
	...
	function shuffleRPC(host, args, callback) {
		console.log("[" + grid.host.id + "] cmd shuffle to " + host.id);
			...
			if (stageIdx == (stage.length - 1)) {
				...
				for (var i = 0; i < map.length; i++) {
					if (grid.host.uuid == worker[i].uuid) {
						...
						shuffleRPC(worker[i], map[i], function(err, res) {
							if (err) throw err;
							console.log('[' + grid.host.id + ']  shuffle response');
						});
					}
				}
	...
	this.processShuffle = function(msg) {
		shuffle[2](msg);	// id of stage to be shuffled
	};
};
