...
TODO:
	...
	- if a stage need not to be computed don't generate it's src
*/
...
function buildTask(worker, array, action) {
	...
	for (var i = 0; i < stage.length; i++)
		stage[i] = {input: [], pipeline: []};

	treewalk(array, null, function (a) {
		// Set transformation args stage and reeverse stage index
		node[a.num] = {args: a.args};
		...
		if (a.inMemory) {
			if (a.stageIdx && lastStage)
				var tmp = 'var input = STAGE_RAM;\n';
			else
				var tmp = 'var input = RAM[' + a.id + '];\n';
			stage[a.stageIdx] = {input: [tmp], pipeline: ['']};
			...
			stage[a.stageIdx].input.push(tmp);
			stage[a.stageIdx].pipeline.push('');
		} else {
			...
			for (var i = 0; i < stage[a.stageIdx].pipeline.length; i++)
				stage[a.stageIdx].pipeline[i] += tmp;
		}
	...
	for (var i = 0; i < stage.length; i++) {
		...
		for (var j = 0; j < stage[i].pipeline.length; j++) {
			code += stage[i].input[i];
			code += 'for (var p in input) {\n' +
				'\tfor (var i = 0; i < input[p].length; i++) {\n' +
				'\t\tvar tmp = input[p][i];\n';
			code += stage[i].pipeline[j];
			if (i == lastStage)
				code += action.run;
			code += '\t}\n}\n';
		}
	...
	var task = taskTemplate.toString().replace('"WORKERS_AND_STAGES"', code);
	return {node: node, action: action, task: task}
}
