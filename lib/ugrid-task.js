...
var taskTemplate = function(grid, ml, STAGE_RAM, RAM, node, action) {	
	...
	function shuffleRPC(host, args, callback) {
		grid.send_cb({cmd: 'request', id: host.id, data: {cmd: 'shuffle', args: args}}, function(err, res) {
			callback(err, res);
		});
	}
