		...
		treewalk(array, null, function (a) {
			...
			for (i = 0; i < worker.length; i++) {
				if (a.transform == 'parallelize') {
					...
					task[i].node[a.num] = {args: [a.args[0][i]]}; // blocks affectés au worker i
			...
			if (a.inMemory) {
				...
				if (a.transform == 'parallelize') {
					...
					// Valable pour un seul bloc
					function hdfsTextFile(num, id, persistent, stageIdx) {
						var file = node[num].args[0][0].file;
						var partitionIdx = [node[num].args[0][0].blockNum];
						var partitionLength = {}; 
						for (var p = 0; p < partitionIdx.length; p++) 
							partitionLength[partitionIdx[p]] = 0;
						if (persistent) {
					...
					stage[a.stageIdx].lineages.push({
						...
						input: hdfsTextFile.toString() + ';' + 
							'hdfsTextFile(' + a.num + ', ' + a.persistent.toString() + ', ' + a.id + ', ' + a.stageIdx + ');'
					});
				}
			} else {
