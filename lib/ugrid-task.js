...
function buildTask(grid, worker, array, action, callback) {
	preBuildTask(grid, worker, array, action, function() {
		...
		treewalk(array, null, function(a) {
			...
			if (a.inMemory) {
				// Discard previous stages
				if (a.dependency == 'wide') {
					var sid = a.stageIdx + 1;
					for (var i = 0; i < sid; i++) delete stageData[i];					
				} else var sid = a.stageIdx;
				// Reset current stage (faux, current *lineages*) to start here
				// Attention: il ne faut pas supprimer tous les lineages, mais seulement ceux
				// partant de la descendance de a
				stageData[sid].lineages = [[{
					num: a.num,
					src_id: a.id,
					type: 'fromRAM',
		...
		for (var i in stageData)
			finalStageData.push(stageData[i]);
		...
		for (i = 0; i < worker.length; i++) {
			...
			if (finalStageData[finalStageData.length - 1].lineages.length == 0) {
				finalStageData[finalStageData.length - 1].lineages = [[{
					num: array.num,
					dest_id: array.id,
					type: 'fromSTAGERAM',
