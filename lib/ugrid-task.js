...
var taskTemplate = function(grid, ml, STAGE_RAM, RAM, lineages, action, callback) {
	...
	function runStage(idx) {
		...
		// console.log('Running stage : ' + (idx + 1) + ' / ' + stage.length);
		// console.log('stageIdx : ' + stageIdx);
		// console.log('stage.length - 1 : ' + (stage.length - 1));
		// console.log('lastStage : ' + lastStage);

		// if (idx >= stage.length)
		// 	throw 'Error in runStage'
	...
	this.processShuffle = function(data) {
		...
		if (++nShuffle == worker.length) {
			nShuffle = 0;
			var tmp = [];
			for (var p in STAGE_RAM)
				tmp.push(STAGE_RAM[p]);
			STAGE_RAM = tmp;
			runStage(++stageIdx);
		} else
