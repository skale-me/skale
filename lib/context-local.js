...
function Context(args) {
  ...
  this.textFile = function (file, opt, nPartitions) {
    ...
    if (u.protocol === 's3:') {
      if (file.slice(-1) === '/')
        return new dataset.TextS3Dir(this, file.slice(5, -1), opt);
      return new dataset.TextS3File(this, file.slice(5), opt);
    }
    if (u.protocol === 'wasb:') {
      if (file.slice(-1) === '/')
        return new dataset.TextAzureDir(this, file.slice(7, -1), opt);
      return new dataset.TextAzureFile(this, file.slice(7), opt);
    }
    if (file.slice(-1) === '/')
      return new dataset.TextDir(this, file, opt);
    if (opt.parquet || file.slice(-8) === '.parquet')
      return new dataset.ParquetFile(this, file, opt);
    if (file.slice(-3) === '.gz')
