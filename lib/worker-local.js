...
function runTask(msg) {
  ...
  task.grid = {host: {}};
  task.mm = mm;
  task.lib = {aws: aws, azure: azure, fs: fs, Lines: Lines, mkdirp: mkdirp, mm: mm, parquet: parquet, readSplit: readSplit, url: url, uuid: uuid, zlib: zlib};
  task.log = log;
...
if (process.env.SKALE_DEBUG > 1) {
  var log =  function() {
    ...
    args.unshift('[worker-' +  process.argv[2] + ' ' + (elapsed[0] + elapsed[1] / 1e9).toFixed(3) + 's]');
