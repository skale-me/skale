...
module.exports = function(arg) {
	...
	this.connect_cb = function(callback) {
		grid = net.connect(port, host, function() {
			...
			byline(grid).on('data', function(d) {
				try {
					var o = JSON.parse(d, function(key, value) {
						if (value === "gridNaN")
							return NaN;
						return value;
					});
					...
					if (o.cmd in grid_events) {	      // command from server
						...
						pending_cmd[o.cmd_id].process(o.data.err, o.data.result);
					} 
				} catch (error) {
	...
	var send = this.send_cb = function(cmd, data, callback) {
		...
		if (callback) {
			o.process = function(error, result, cmd_id) {
