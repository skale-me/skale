...
function UgridClient(arg) {
	...
	function send_cb(o, callback, uuid) {
		if (!o.cid) {
		...
		if (o.id > 3 && !hostUuid[o.id]) {
			console.log(o.id);
			console.log("should ask for new id");
		}
		o.from = myId;
		sock.write(ugridMsg.encode(o));
	}

	function subscribe(publishers, callback) {
		var v;
		for (v in publishers) {
			if (subscriptions.indexOf(publishers[v].uuid) < 0)
				subscriptions.push(publishers[v].uuid);
		}
		send_cb({cmd: 'subscribe', data: publishers}, callback);
	};
	...
	this.unsubscribe = function unsubscribe(publishers) {
		var i, v;
		for (v in publishers) {
			i = subscriptions.indexOf(publishers[v].uuid);
			if (i >= 0)
