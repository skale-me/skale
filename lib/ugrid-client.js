...
Consumer.prototype._transform = function (chunk, encoding, done) {
	...
	if (msg.cmd == 'reply') {
		...
		// console.log('\n# Received')
		// console.log(msg)
		//this.client.emit(msg.cmd, msg, done);
...
WriteStream.prototype._transform = function (chunk, encoding, done) {
	this.client.send(this.dest, {cmd: 'request', data: {cmd: 'stream', stream: this.name, data: chunk}}, function (err, res) {
		done();
	});
};

//function ReadStream (client, name, dest) {
