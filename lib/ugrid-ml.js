...
var Dataset = require('./ugrid-array.js').Dataset;
var Partition = require('./ugrid-array.js').Partition;

var thenify = require('thenify');
var ml = {};
module.exports = ml;

ml.RandomSVMData = function(uc, N, D, seed, nPartitions) {
	...
	this.iterate = function(job, p, pipeline, done) {
		var ml = this.ml;
		var rng = new ml.Random(this.splits[p].seed), buffer;
		var D = this.D;
...
ml.KMeans = function (data, nClusters, initMeans) {
	...
	this.closestSpectralNorm = function (element, args) {
		...
		for (var i = 0; i < args.means.length; i++) {
			...
			for (var j = 0; j < element.length; j++)
				sn += Math.pow(element[j] - args.means[i][j], 2);
			if (sn < smallestSn) {
				smallestSnIdx = i;
				smallestSn = sn;
			}
	...
	this.train = thenify(function(nIterations, callback) {
		...
		function iterate() {
			console.time(i);
			data.map(self.closestSpectralNorm, {means: self.means})
