...
module.exports.randomSVMLine = function(rng, D) {
	var data = rng.randn(D + 1);
	data[0] = Math.round(Math.abs(data[0])) * 2 - 1; 
	return data;
}

module.exports.LogisticRegression = function (data, D, N) {
	var self = this;
	var rng = new Random();
	this.w = rng.randn(D);
...
module.exports.KMeans = function (data, nClusters) {
	var seed = 1;
	var maxMse = 0.0001;
	this.mse = [];
	var means, D;
		...
		function iterate() {
			data.map(self.closestSpectralNorm, [self.means])
				...
				.collect(function(err, newMeans) {
					// Compute squared distance between previous and current means
					var dist = 0;
