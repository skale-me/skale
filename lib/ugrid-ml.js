...
ml.LinearSVM = function (data, D, N, w) {
	...
	this.train = thenify(function(nIterations, callback) {
		...
		function hingeLossGradient(p, w) {
		...
		function iterate() {
			console.time(i);
			data.map(hingeLossGradient, [self.w]).reduce(sum, zeros(D), function(err, gradient) {
				console.timeEnd(i);
...
ml.LinearRegression = function (data, D, N, w) {
	...
	this.train = thenify(function(nIterations, callback) {
		...
		function squaredLossGradient(p, w) {
			var grad = [], dot_prod = 0, label = p[0], features = p[1];
			for (var i = 0; i < features.length; i++)
				dot_prod += features[i] * w[i];
			for (var i = 0; i < features.length; i++) 
		...
		function iterate() {
			console.time(i);
			data.map(squaredLossGradient, [self.w]).reduce(sum, zeros(D), function(err, gradient) {
				console.timeEnd(i);
				for (var j = 0; j < self.w.length; j++)
					self.w[j] -= gradient[j] / (N * Math.sqrt(i + 1));
				if (++i == nIterations) callback();
