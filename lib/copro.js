...
if (webid) {
	console.log = function (d) {
		process.stdout.write(JSON.stringify({data: d, file: uc._file}) + '\n');
		stdout.write(d + '\n');
	};
	plot = function (data) {
		process.stdout.write(JSON.stringify({plot: data, file: uc._file}) + '\n');
		fs.writeFile(plotfile, JSON.stringify(data), function () {});
	};
	console.error(plot)
}
...
co(function *() {
	...
	input.on('data', function(msg) {
		...
		if (webid) {
			...
			if (msg.file) {
				uc._file = msg.file;
				var outfile = msg.file.replace('in', 'out').replace('.js', '');
				plotfile = outfile.replace('out', 'plot');
