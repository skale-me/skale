...
function encode(msg) {
	var str = JSON.stringify(msg), len = Buffer.byteLength(str), buf = new Buffer(len + 8);
...
function bencode(chunk, id, streamid) {
	...
	if (chunk.length)
		return Buffer.concat([header, chunk], chunk.length + 9);
	return header;
}
...
Consumer.prototype._transform = function (chunk, encoding, done) {
	var streamid = chunk[8];
	if (this.streams[streamid]) {
		if (chunk.length == 9) return this.streams[streamid].end(done);
...
function ReadStream (client, name, opt) {
	if (!(this instanceof ReadStream))
		return new ReadStream(client, name, opt);
	stream.Transform.call(this, {objectMode: true});
	this.client = client;
	this.name = name;
	this.opt = opt || {};
...
function Client(opt, callback) {
	...
	if (!opt.access && !inBrowser) opt.access = process.env.SKALE_ACCESS;
...
Client.prototype.createStreamFrom = function (client, msg) {
	...
	this.consumer.streams[sid] = s = new stream.PassThrough();
	s.once('end', function () {self.consumer.streams[sid] = undefined});
	msg.streamid = sid;
