...
function UgridArray(grid, worker, child, transformType, transform, args) {
	...
	this.reduceByKey = function (reducer, initVal) {
		var a = new UgridArray(grid, worker, [this], 'wide', 'reduceByKey', [initVal]);
		a.pipelineSource = function () {
			var tmp = 'if (res[tmp[0]] == undefined)' +
				'res[tmp[0]] = [[tmp[0], JSON.parse(JSON.stringify(node[' + this.num + '].args[0]))]];';
			tmp += 'res[tmp[0]][0][1] = transform[' + this.num + '](res[tmp[0]][0][1], tmp[1]);';
			return tmp;
		}
		a.transformSource = function () {
		...
		a.shuffleSource = function () {
			var shuffle = function (msg) {
				console.log('[' + grid.host.id + '] Received shuffle: ' + util.inspect(msg));
				nShuffle++;
	...
	this.groupByKey = function () {
		var a = new UgridArray(grid, worker, [this], 'wide', 'groupByKey', []);
		a.pipelineSource = function () {
			var tmp = 'if (res[tmp[0]] == undefined) res[tmp[0]] = [[]];';
			tmp += 'res[tmp[0]][0].push(tmp);';
			return tmp;
		}
		a.shuffleSource = function () {
			var shuffle = function (msg) {
