...
function UgridArray(grid, worker, child, transformType, transform, args) {
	...
	function rpc(grid, cmd, workerNum, args, callback) {
		grid.send_cb({cmd: 'request', id: worker[workerNum].id, data: {cmd: cmd, args: args}}, function(err, res) {
			callback(err, res, workerNum);
		});
		// grid.send_cb({cmd: 'request', id: worker[workerNum].id, data: {cmd: 'setTask', args: args}}, function(err, res) {
		// 	console.log('worker ready')
		// 	n++;
		// 	console.log(n + ' / ' + worker.length);
		// 	if (n == worker.length) {
		// 		console.log('Starting task');
		// 		grid.send_cb({cmd: 'request', id: worker[workerNum].id, data: {cmd: 'runTask'}}, function(err, res) {
		// 			callback(err, res, workerNum);
		// 		});
		// 	}
		// });
	...
	this.collect_cb = function(callback) {
		...
		for (var i = 0; i < worker.length; i++) {
			rpc(grid, 'setTask', i, task[i], function(err, res) {
				if (++n == worker.length) {
					for (var j = 0; j < worker.length; j++) {
						rpc(grid, 'runTask', j, null, function(err, res, from) {
	...
	this.sample_cb = function(fraction, callback) {
		...
		var nAnswer = 0, result = [], N = 0, n = 0;
		for (var i = 0; i < worker.length; i++) {
			rpc(grid, 'setTask', i, task[i], function(err, res, workerNum) {
				if (++n == worker.length) {
					for (var j = 0; j < worker.length; j++) {
						rpc(grid, 'runTask', j, null, function(err, res) {
							N += res.N;
