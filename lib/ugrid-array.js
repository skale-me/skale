...
function UgridArray(uc, child, transform, args, src) {
	var self = this;
	...
	this.map = function (mapper, args) {
		return new UgridArray(uc, [this], 'map', args, mapper.toString());
	};

	this.flatMap = function (mapper, args) {
		return new UgridArray(uc, [this], 'flatMap', args, mapper.toString());
	};

	this.mapValues = function (mapper, args) {
		return new UgridArray(uc, [this], 'mapValues', args, mapper.toString());
	};

	this.flatMapValues = function (mapper, args) {
		return new UgridArray(uc, [this], 'flatMapValues', args, mapper.toString());
	};

	this.filter = function (filter, args) {
		return new UgridArray(uc, [this], 'filter', args, filter.toString());
	};
	...
	this.reduceByKey = function (reducer, initValue, args) {
		...
		return new UgridArray(uc, [this], 'reduceByKey', [initValue, args], reducer.toString());
	};

	this.sample = function (withReplacement, frac, seed) {
		return new UgridArray(uc, [this], 'sample', [withReplacement, frac, seed || 1]);
	};
	...
	this.union = function (other) {
		...
		return new UgridArray(uc, [this, other], 'union', [other.id]);
	};

	this.join = function (other) {
		if (other.id == this.id) return this;
		return new UgridArray(uc, [this, other], 'join', [other.id, 'inner']);
	};

	this.leftOuterJoin = function (other) {
		if (other.id == this.id) return this;
		return new UgridArray(uc, [this, other], 'join', [other.id, 'left']);
	};

	this.rightOuterJoin = function (other) {
		if (other.id == this.id) return this;
		return new UgridArray(uc, [this, other], 'join', [other.id, 'right']);
	};

	this.coGroup = function (other) {
		if (other.id == this.id) return this;
		return new UgridArray(uc, [this, other], 'coGroup', [other.id]);
	};

	this.crossProduct = function (other) {
		if (other.id == this.id) return this;
		return new UgridArray(uc, [this, other], 'crossProduct', [other.id]);
	};
...
function buildJob(uc, opt, array, action, callback, initValue, combiner) {
	...
	function cout(a, callback) {
		if (nodeNums.indexOf(a.num) != -1) return callback();	// if already in return
		nodeNums.push(a.num);
