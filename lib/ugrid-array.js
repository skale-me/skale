/*
	TODO:
		- textFile
		- parrallelize (only for one worker now)
		- filter
		- flatMap
		- sample
		- lookup
		- reduceByKey (découper le vecteur avec du hashcoding avant d'envoyer la tache)
		- takeSample (mieux gérer le tirage au sort des éléments)
	...
	function buildTask(array, action) {
		...
		for (var i = 0; i < lineages.length; i++) {
			for (var j = 0; j < lineages[i].length; j++) {
				...
				// console.log(lineages[i][j].transform)
				// console.log('lineages[i][j].stage : ' + lineages[i][j].stage);
				// console.log('j :' + j);
				// console.log('stage[lineages[i][j].stage - 1] : ' + stage[lineages[i][j].stage - 1])
				// console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
		...
		for (var i = 0; i < stage.length; i++) {
			if (i == (stage.length - 1))
				stageStr[i] = buildStage(stage[i], i, stage.length, action);
			else
				stageStr[i] = buildStage(stage[i], i, stage.length);
		}

		// Write task constructor
		var tmp = '';
		for (var i = 0; i < worker.length; i++)
			tmp += '\tworker[' + i + '] = "' + worker[i] + '";\n';
		for (var i = 0; i < stageStr.length; i++)
			tmp += '\tstage[' + i + '] = ' + stageStr[i] + ';\n';

		var lastStr = taskTemplate.toString().replace('"WORKERS_AND_STAGES"', tmp);
		...
		function buildStage(lineages, stageIdx, nStages, action) {
			...
			for (var l = 0; l < lineages.length; l++) {
				...
				if (lineages[l][0].srcId) {
					...
					// Si (dernier stage) OU (pas le dernier stage ET )
					// console.log(lineages[l][0].transformType);
				} else {
				...
				for (var i = 0; i < lineages[l].length; i++) {
					...
					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
					// Display Dependency type HERE
					// console.log(lineages[l][i].transformType);
					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
