...
function UgridArray(grid, child, dependency, transform, args, src) {
	...
	this.getNode = function (wid) {
		return {
			args: this.getArgs ? this.getArgs(wid) : JSON.parse(JSON.stringify(this.args)),
			id: this.id,
			type: this.transform,
			dependency: this.dependency,
			persistent: this.persistent,
			inMemory: this.inMemory,
			target_cnt: this.child.length,
			stageNode: this.stageNode,
			num: this.num,
			src: src,
			child: this.child.map(function(e) {return e.num;}),
			anc: this.anc && this.anc.num,
			visits: 0
		};
	}
...
function buildJob(grid, opt, array, action, callback, initValue, processPartialResult, postProcess) {
	...
	co(function *() {
		...
		yield treewalk(array, thenify(function(a, callback) {
			...
			// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			// MODIFIER EGALEMENT LE STAGE_NODE PENDANT LA REMONTEE ICI      <<<<< IMPORTANT !!!!
			// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
