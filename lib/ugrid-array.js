...
function buildJob(uc, opt, array, action, callback, initValue, combiner) {
	...
	asyncTreewalk(array, cin, cout, function() {
		...
		uc.on('stream.' + jobId, function(data, done) {
			stream.write(data, done);
			// stream.write(JSON.stringify(data) + '\n', done);	// to handle process.stdout WriteStream
		});
		...
		// Déploiement du job vers l'ensemble des workers et synchronisation du run
		var n = 0;
		for (var i = 0; i < uc.worker.length; i++)
