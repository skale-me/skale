...
function TextFile(uc, file, nPartitions) {
	...
	this.iterate = function(task, p, pipeline, done) {
		...
		task.lib.readSplit(this.splits, this.splits[p].index, processLine, done, function (path, opt) {
...
function Cartesian(uc, dependencies) {
	...
	this.iterate = function(task, p, pipeline, done) {
		...
		var a1 = task.lib.fs.readFileSync(this.shufflePartitions[p1].files.path, {encoding: 'utf8'}).split('\n');
		var a2 = task.lib.fs.readFileSync(this.shufflePartitions[p2].files.path, {encoding: 'utf8'}).split('\n');
