...
function UgridArray(grid, worker, child, dependency, transform, args, src) {
	...
	this.count = ugridify(function (opt, callback) {
		...
		buildJob(grid, worker, this, {fun: 'count', args: [], init: 0, jobId: jobId}, function (job, nTarget, streams) {
			...
			sendJob(job, jobId, streams, function (err, msg, done) {
				...
				if (++nAnswer == nTarget) {
					...
					for (var s in streams)
						try {streams[s].dispatch.done();} catch (e) {};
					if (opt.stream) {
	...
	this.reduce = ugridify(function (reducer, aInit, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'reduce', args: [aInit], src: reducer.toString(), init: aInit, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, dones = [];
	...
	this.take = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'take', args: [num], init: 0, jobId: jobId}, function (job, nTarget, streams) {
	...
	this.top = ugridify(function (num, opt, callback) {
		...
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'top', args: [num, numsort], init: 0, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, result = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
	...
	this.takeOrdered = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'takeOrdered', args: [num, numsort], init: 0, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, result = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				result = result.concat(res);
	...
	this.collect = ugridify(function (opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		//if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'collect', args: [], init: {}, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, vect = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				for (var p in res) vect = vect.concat(res[p]);
				if (++nAnswer < nTarget) return;
				grid.jobs[jobId].iteration++;
				for (var s in streams)
					try {streams[s].dispatch.done();} catch (e) {};
				if (opt.stream) {
	...
	this.lookup = ugridify(function (key, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'lookup', args: [key], init: {}, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, vect = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				var i, p;
				dones.push(done);
				for (p in res) vect = vect.concat(res[p]);
				if (++nAnswer == nTarget) {
					grid.jobs[jobId].iteration++;
					for (var s in streams)
						try {streams[s].dispatch.done();} catch (e) {};
	...
	this.countByValue = ugridify(function (opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'countByValue', args: [], init: {}, jobId: jobId}, function (job, nTarget, streams) {
	...
	this.forEach = ugridify(function (eacher, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(grid, worker, this, {fun: 'forEach', args: [], src: eacher.toString(), init: {}, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				if (++nAnswer == nTarget) {
...
function buildJob(grid, worker, array, action, callback) {
	...
	co(function *() {
		...
		var nTarget = worker.length;
		if (onlyStreams) {
			for (var i = 0; i < streamUgridArrays.length; i++) {
				if (streamUgridArrays[i].config.N >= worker.length) {
					nTarget = worker.length;
					break;
				}
				nTarget = Number(streamUgridArrays[i].config.N);
			}
		}
		...
		for (i = 0; i < worker.length; i++) {
			...
			job[i].actionData.maxWorker = nTarget;
		}

		// TODO
		// ici on sait si le job ne comprends que des streams ou pas
		grid.jobs[action.jobId].onlyStreams = onlyStreams;
		grid.jobs[action.jobId].iteration = 0;		
		// Au moment du flush du dispatcher dans le cas d'une fin de stream input
		// si (grid.job[jobId].onlyStreams || (grid.job[jobId].iteration > 0)) on peut envoyer ignore true

		callback(job, nTarget, streamUgridArrays);
	}).catch(onError);
}
