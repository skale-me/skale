...
function UgridArray(uc, child, dependency, transform, args, src) {
	...
	this.collect = function () {
		return buildJob(uc, {stream: true}, this, {fun: 'collect'}, null, null);
	};
...
function buildJob(uc, opt, array, action, callback, initValue, combiner) {
	...
	var streamUgridArrays = [];										// list of DStream members of current job
	var jobId = action.jobId = uc.jobId++;							// index of current job
	var stream = opt._stream || new uc.createReadStream(jobId);		// user redable stream instance
	var stageNode;													// Stage Node temprary var for treewalk
	var job = new Array(uc.worker.length);							// Array of job data to be sent to workers
	...
	function postTreewalk() {
		...
		// Collect stream data event
		uc.on('collect', function (msg) {
			var lines = new Lines();
			fs.createReadStream(msg.path).pipe(lines);
			lines.on('data', function(data) {stream.write(JSON.parse(data));});
		});

		// Aggregate stream data event
		uc.on(jobId, function(res, done) {
			dones.push(done);
			if (combiner != undefined) mainResult = combiner(mainResult, res);
			if (++nAnswer < uc.worker.length) return;
			if (!opt.stream) {
				callback(null, mainResult);
				unlock();
			} stream.write(mainResult, unlock);
			function unlock() {
