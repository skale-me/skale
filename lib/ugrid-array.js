...
function UgridArray(grid, worker) {
	...
	function buildTask(array, action) {
		...
		// Step 3: Construction des stages
		...
		for (var i = 0; i < lineages.length; i++) {
			for (var j = 0; j < lineages[i].length; j++) {
				...
				// console.log('\n# debug ugrid-array stage gen')
				// console.log(lineages[i][j].transform + ', stage :' + lineages[i][j].stage + ' / ' + nTotalStage);
				// console.log();
			}
		}
		...
		var taskStr = stageStr[0];
		// var taskStr = buildStage(lineages, action);

		// Construire la structure de donnée finale
		// function task() {
		// 	this.stageIdx = 0;
		// 	this.stage = [];
		// 	this.stage[0] = stageStr[0];
		// 	this.stage[N] = stageStr[N];
		// }
		...
		function buildStage(lineages, action) {
			...
			for (var l = 0; l < lineages.length; l++) {
				...
				if (lineages[l].length == 0) {
					taskStr += 'for (var p = 0; p < RAM[action.srcId].length; p++)\n';
				...
				for (var i = 0; i < lineages[l].length; i++) {
					if (lineages[l][i].transform == 'loadTestData')
						continue;
				...
				// -------------------------------------------------------------------------- //
				taskStr += 'for (var p = 0; p < input.length; p++) {\n';
			...
			return 'function runTask(ml, STAGE_RAM, RAM, lineages, action){\n' + taskStr + '}';			
		}

		return {lineages: lineages, action: action, taskStr: taskStr, lastStr: lastStr}
	}
