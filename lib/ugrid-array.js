...
function UgridArray(grid, worker, child, dependency, transform, args, src) {
	...
	this.join = function (withArray) {
		if (withArray.id == this.id) return this;
		var a = new UgridArray(grid, worker, [this, withArray], 'wide', 'join', [withArray.id]);

		a.pipelineSource = function () {
			var tmp = 'if (res[tmp[0]] == undefined) res[tmp[0]] = [];';
			tmp += 'res[tmp[0]].push(tmp);';
			return tmp;
		}

		a.shuffleSource = function () {
			var shuffle = function () {
				throw 'join shuffle to be implemented'
			}
