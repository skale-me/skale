...
function UgridArray(grid, worker, child, dependency, transform, args, src) {
	...
	function sendJob(job, jobId, streamUgridArrays, callback) {
		//console.log('in sendJob, jobId=%j', jobId);
		//console.log('in sendJob, streamUgridArrays: %j', streamUgridArrays);
	...
	this.collect = thenify(function (callback) {
		var jobId = grid.jobId++;
		grid.jobs[jobId] = {count: 0, stream: new grid.createReadStream(jobId), finished: false};
		buildJob(worker, this, {fun: 'collect', args: [], init: {}, jobId: jobId}, function (job, streams) {
			var nAnswer = 0, vect = [], dones = [];

			console.log('in collect, streams: %j', streams);
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				for (var p in res) vect = vect.concat(res[p]);
				console.log('in collect callback');
				if (++nAnswer == worker.length) {
					callback(null, vect);
					vect = [];
					nAnswer = 0;
	...
	this.collectStream = function () {
		var jobId = grid.jobId++;
		var stream = new grid.createReadStream(jobId);
		grid.jobs[jobId] = {count: 0, stream: stream};
		buildJob(worker, this, {fun: 'collect', args: [], init: {}, jobId: jobId}, function (job, streams) {
			var nAnswer = 0, vect = [], dones = [];

			console.log('in collectStream, streams: %j', streams);
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				for (var p in res) vect = vect.concat(res[p]);
				if (++nAnswer == worker.length) {
					//callback(null, vect);
					//stream.emit('data', vect);
					stream.write(vect);
					vect = [];
					nAnswer = 0;
					for (var i in dones) dones[i]();
