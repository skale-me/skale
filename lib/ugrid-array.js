...
function UgridArray(uc, child, dependency, transform, args, src) {
	...
	for (var i = 0; i < child.length; i++) child[i].anc = this;	// set child ancestor

	this.getArgs = function(callback) {		// By default duplicate same args for each worker
		callback(uc.worker.map(function(e) {return JSON.parse(JSON.stringify(self.args))}));
	}
	...
	this.countByValue = ugridify(function (opt, callback) {
		...
		return buildJob(uc, opt, this, action, callback, init, reducer, postReducer);
	});
...
function buildJob(uc, opt, array, action, callback, initValue, processPartialResult, postProcess) {
	...
	for (var i = 0; i < job.length; i++) 
		job[i] = {node: {}, worker: uc.worker, actionData: action};
	...
	function cout(a, callback) {
		if ((stageNode == undefined) && (a.transform == 'union') && !a.inMemory){	// IDEM si union sous STAGE NODE
		...
		if (a.transform == 'stream') {
			...
			uc.streams[a.streamId].push(action.jobId);
			// add streamId to list of active input streams associated to jobId
			uc.jobs[action.jobId].activeInputStreams.push(a.streamId);
			streamUgridArrays.push(a);
		}

		if (((a.child.length == 0) && (a.transform != 'stream')) || a.inMemory) // QUID SI IN MEMORY ?
			uc.jobs[action.jobId].onlyStreams = false;
