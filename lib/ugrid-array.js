...
function UgridArray(grid, worker, child, dependency, transform, args, src) {
	...
	this.count = ugridify(function (opt, callback) {
		...
		buildJob(worker, this, {fun: 'count', args: [], init: 0, jobId: jobId}, function (job, onlyStreams, streams) {
	...
	this.reduce = ugridify(function (reducer, aInit, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'reduce', args: [aInit], src: reducer.toString(), init: aInit, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, dones = [];
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
	...
	this.take = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'take', args: [num], init: 0, jobId: jobId}, function (job, onlyStreams, streams) {
	...
	this.top = ugridify(function (num, opt, callback) {
		...
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'top', args: [num, numsort], init: 0, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, result = [], dones = [];
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
	...
	this.takeOrdered = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'takeOrdered', args: [num, numsort], init: 0, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, result = [], dones = [];
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
	...
	this.collect = ugridify(function (opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'collect', args: [], init: {}, jobId: jobId}, function (job, onlyStreams, streams) {
	...
	this.lookup = ugridify(function (key, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'lookup', args: [key], init: {}, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, vect = [], dones = [];
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
	...
	this.forEach = ugridify(function (eacher, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'forEach', args: [], src: eacher.toString(), init: {}, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, dones = [];
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
...
function buildJob(worker, array, action, callback) {
	...
	co(function *() {
		...
		callback(job, onlyStreams, streamUgridArrays);
	}).catch(onError);
}
