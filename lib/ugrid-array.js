...
function UgridArray(grid, worker, child, transformType, transform, args) {
	...
	function rpc(grid, workerNum, args, callback) {
		grid.send_cb({cmd: 'request', id: worker[workerNum].id, data: {cmd: 'task', args: args}}, function(err, res) {
	...
	// Return dataset entries, ordered
	this.collect_cb = function(callback) {
		...
		for (var i = 0; i < worker.length; i++)
			rpc(grid, i, task[i], function(err, res, from) {
				res.map(function(n) {
					// NOT SURE ABOUT INDEX CONSTRUCTION HERE !!!!!
					n.idx = n.pidx * (n.nPart - 1) + (n.idx * worker.length + from);
				});
