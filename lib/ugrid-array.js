...
function UgridArray(grid, worker, child, dependency, transform, args, src) {
	...
	this.count = ugridify(function (opt, callback) {
		...
		buildJob(worker, this, {fun: 'count', args: [], init: 0, jobId: jobId}, function (job, nTarget, streams) {
	...
	this.reduce = ugridify(function (reducer, aInit, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'reduce', args: [aInit], src: reducer.toString(), init: aInit, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, dones = [];
	...
	this.take = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'take', args: [num], init: 0, jobId: jobId}, function (job, nTarget, streams) {
	...
	this.top = ugridify(function (num, opt, callback) {
		...
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'top', args: [num, numsort], init: 0, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, result = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
	...
	this.takeOrdered = ugridify(function (num, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'takeOrdered', args: [num, numsort], init: 0, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, result = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				result = result.concat(res);
	...
	this.collect = ugridify(function (opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'collect', args: [], init: {}, jobId: jobId}, function (job, nTarget, streams) {
	...
	this.lookup = ugridify(function (key, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'lookup', args: [key], init: {}, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, vect = [], dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				var i, p;
				dones.push(done);
	...
	this.countByValue = ugridify(function (opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'countByValue', args: [], init: {}, jobId: jobId}, function (job, onlyStreams, streams) {
			var nAnswer = 0, tmp = {}, dones = [], i;
			var nTarget = worker.length;
			if (onlyStreams) {
				for (var i = 0; i < streams.length; i++) {
					if (streams[i].config.N >= worker.length) {
						nTarget = worker.length;
						break;
					}
					nTarget = Number(streams[i].config.N);
				}
			}
	...
	this.forEach = ugridify(function (eacher, opt, callback) {
		...
		grid.jobs[jobId] = {count: 0, stream: stream, finished: false};
		// if (!opt.stream) stream.once('end', callback);
		buildJob(worker, this, {fun: 'forEach', args: [], src: eacher.toString(), init: {}, jobId: jobId}, function (job, nTarget, streams) {
			var nAnswer = 0, dones = [];
			sendJob(job, jobId, streams, function (err, res, done) {
				dones.push(done);
				if (++nAnswer == nTarget) {
