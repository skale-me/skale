...
function UgridArray(grid, child, dependency, transform, args, src) {
	...
	this.getArgs = function(callback) {		// By default duplicate same args for each worker
		callback(grid.worker.map(function(e) {return JSON.parse(JSON.stringify(self.args))}));
	}
	...
	this.takeSample = ugridify(function (withReplacement, num, seed, opt, callback) {
		...
		return buildJob(grid, opt, this, action, function(err, result) {
			...
			var stream = grid.jobs[action.jobId].stream;
...
function buildJob(grid, opt, array, action, callback, initValue, processPartialResult, postProcess) {
	...
	grid.jobs[jobId] = {count: 0, stream: stream, finished: false, onlyStreams: true, iteration: 0};
	action.jobId = jobId;
	action.target_cnt = 1;
	for (var i = 0; i < job.length; i++) 
		job[i] = {node: {}, worker: grid.worker, actionData: action};
	...
	function cout(a, callback) {
		...
		if ((a.child.length == 0) && (a.transform != 'stream')) 
			grid.jobs[action.jobId].onlyStreams = false;
	...
	asyncTreewalk(array, cin, cout, function() {
		...
		grid.on(jobId, function(res, done) {
			dones.push(done);
