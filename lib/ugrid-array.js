...
function SortBy(uc, dependencies, keyFunc, ascending, numPartitions) {
	...
	this.getPartitions = function(done) {
		if (this.partitions == undefined) {
			var self = this, P = Math.max(numPartitions || 1, this.dependencies[0].partitions.length);
			this.dependencies[0].sample(false, 0.5).collect().toArray(function(err, result) {
				function compare(a, b) {
					if (self.keyFunc(a) < self.keyFunc(b)) return -1;
					if (self.keyFunc(a) > self.keyFunc(b)) return 1;
					return 0;
				}
				result.sort(compare);
				self.partitioner = new RangePartitioner(P, result);				
				self.partitions = [];
				for (var p = 0; p < P; p++) self.partitions.push(new Partition(self.id, p));
				done();
			})
		} else done();
	}
	...
	this.transform = function(context, data) {
		for (var i = 0; i < data.length; i++) {
			var pid = this.partitioner.getPID(this.keyFunc(data[i]));
			if (this.buffer[pid] == undefined) this.buffer[pid] = [];
