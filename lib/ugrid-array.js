...
function UgridArray(grid, child, dependency, transform, args, src) {
	...
	this.takeOrdered = ugridify(function (num, opt, callback) {
		...
		var action = {fun: 'takeOrdered', args: [num, numsort], init: 0};
		var init = [];
		var reducer = function(a, b) {
			a = a.concat(b);
			a = a.sort(numsort);
...
function buildJob(grid, opt, array, action, callback, initValue, processPartialResult, postProcess) {
	...
	co(function *() {
		...
		////////////////////////////////////////////////////////////
		// AJOUT du lid et du sid de chaque noeud pour simplifier coté worker
		////////////////////////////////////////////////////////////
		for (var sid = 0; sid < finalStageData.length; sid++) {
			var isLastStage = (sid == finalStageData.length - 1);
			// console.log('Stage ' + sid + ', lineages');
			// console.log(finalStageData[sid].lineages);
			for (var lid = 0; lid < finalStageData[sid].lineages.length; lid++) {
				// console.log('Lineage ' + lid + ', node path');
				// console.log(finalStageData[sid].lineages[lid]);
				// ajout du lid et du sid sur tous les noeuds source du graph
				// console.log('Source node num = ' + finalStageData[sid].lineages[lid][0]);
				job.map(function(jw) {
					jw.node[finalStageData[sid].lineages[lid][0]].lid = lid;
					jw.node[finalStageData[sid].lineages[lid][0]].node_path = finalStageData[sid].lineages[lid];
					jw.node[finalStageData[sid].lineages[lid][0]].inLastStage = isLastStage;
				});
				// ajouter à tous les noeuds du lineages le sid correspondant
			}
		}
		// console.log(job[0].node);
		////////////////////////////////////////////////////////////
