...
function UgridArray(uc, child, dependency, transform, args, src) {
	...
	// this.reduce = function (reducer, aInit, opt, callback) {
	// 	return this.aggregate(reducer, reducer, aInit, opt, callback)
	// };
	...
	// this.count = function(opt, callback) {
	// 	return this.aggregate(function(a, b) {return ++b}, function(a, b) {return a + b}, 0, opt, callback);
	// };
	...
	this.saveAsTextFile = ugridify(function (path, opt, callback) {
		opt = opt || {};
		if (arguments.length < 3) callback = opt;
		var action = {fun: 'saveAsTextFile', args: [path], init: {}};
		var init = null;
		var reducer = function(a, b) {return null;}
		return buildJob(uc, opt, this, action, callback, init, reducer);
	});

	this.take = ugridify(function (num, opt, callback) {					// cas particulier de collect ?
		opt = opt || {};
		if (arguments.length < 3) callback = opt;
	...
	this.top = ugridify(function (num, opt, callback) {						// cas particulier de collect ?
		opt = opt || {};
		if (arguments.length < 3) callback = opt;
	...
	this.takeOrdered = ugridify(function (num, ordering, opt, callback) {	// cas particulier de collect ?
		opt = opt || {};
		if (arguments.length < 4) callback = opt;
...
function buildJob(uc, opt, array, action, callback, initValue, reducer, postReducer) {
	...
	action.targetPathCount = 1;		// ligne en trop ?
	uc.jobs[jobId] = {
		count: 0,
		stream: stream,
		finished: false,
		...
		iteration: 0,
		activeInputStreams: [],
		streamsToLaunch: streamUgridArrays
	};
