...
module.exports.UgridJob = function(request, grid, RAM, msg) {
	...
	for (var i = 0; i < stageData.length; i++)
		stage[i] = new Stage(worker, grid, node, RAM, i, stageData[i], nextStage, master_uuid, dones);
...
function Stage(worker, grid, node, RAM, stageIdx, stageData, nextStage, master_uuid, dones) {
	...
	if (action) {
		var stream = grid.createWriteStream('0', master_uuid);
	}
	for (var wid = 0; wid < worker.length; wid++)
		if (worker[wid].uuid == grid.host.uuid) break;

	for (var l = 0; l < lineages.length; l++)
		lineage[l] = new sources[node[lineages[l][0]].type](l, grid, worker, state, node, RAM, lineages[l], action);

	this.run = function () {
		for (var l = 0; l < lineage.length; l++) {
			try {lineage[l].run(function(streamIdx, done) {
				if (done)
			...
			catch (err) {throw new Error("Lineage error, " + err);}
		}
	}
