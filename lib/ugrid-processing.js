		...
		for (var l = 0; l < this.source.length; l++) {
			try {
				this.source[l].run(function() {
					// console.log('Fin de lineage dans stage n°' + self.sid);
					if ((++self.cnt < self.target_cnt) || self.locked) return;					
					// console.log('Passage de la barrière du stage n°' + self.sid);					
					if (!param.isLastStage) {
						// console.log('Shuffle des data du stage n°' + self.sid);
						node[shuffleNum].transform.tx_shuffle();
						// console.log('nShuffle = ' + self.nShuffle)
						if (self.nShuffle == app.worker.length) {
							// console.log('Invocation du stage n°' + (self.sid + 1));
