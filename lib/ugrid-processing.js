...
function Stage(worker, grid, node, RAM, stageIdx, stageData, nextStage, master_uuid) {
	...
	for (var l = 0; l < lineages.length; l++)
		lineage[l] = new sources[node[lineages[l][0]].type](l, grid, worker, state, node, RAM, lineages[l], fun_action);
			...
			try {lineage[l].run(function() {
				...
				if (fun_action) {
					...
					if ((wid == 0) || action.unlocked) {
						grid.send(master_uuid, {cmd: '0', result: fun_action.result});
						fun_action.reset();
						if (worker[wid + 1])
							grid.request(worker[wid + 1], {cmd: 'action', args: 'TEST'}, 
	...
	this.processAction = function (msg) {
		if (action.finished) {
			...
			if (worker[wid + 1])
				grid.request(worker[wid + 1], {cmd: 'action', args: 'TEST'}, function(err) {if (err) throw new Error(err);});
		}
