...
var UgridSource = {
	...
	stream: function(grid, app, job, stage, param) {
		...
		grid.on(streamIdx + '.end', function(ignore, done) {
			...
			// TODO
			// Si tous les streams du job sont finis et qu'il n'y a pas d'autres sources
			// de données actives dans le job (parallelize, file, etc.) alors
			// il faut ignorer le prochain envoi de résultat
			// var mustIgnore = ignore;
			for (var i in app.completedStreams) {
				if (app.completedStreams[i] == false) {
					ignore = false;
					break;
				}
			}
