...
module.exports.UgridJob = function(request, grid, RAM, msg) {
	...
	for (i = 0; i < stageData.length; i++)
		stage[i] = new Stage(completedStreams, worker, grid, node, RAM, i, stageData[i], nextStage, master_uuid, dones);
...
function Stage(completedStreams, worker, grid, node, RAM, stageIdx, stageData, nextStage, master_uuid, dones) {
	...
	var action = stageData.action ? new rdd[stageData.action.fun](grid, stageData.action) : null;
	var lineage = [];

	if (action) {
		console.log('stageData.action: %j', stageData.action);
		var stream = grid.createWriteStream(stageData.action.jobId, master_uuid);
	}
	for (var wid = 0; wid < worker.length; wid++)
		if (worker[wid].uuid == grid.host.uuid) break;

	for (var l = 0; l < lineages.length; l++)
		lineage[l] = new sources[node[lineages[l][0]].type](completedStreams, l, grid, worker, state, node, RAM, lineages[l], action);
				...
				lineage[l].run(function(streamIdx, done) {
					...
					if (action) {
						...
						if ((wid === 0) || action.unlocked) {
							...
							action.reset();
							if (worker[wid + 1])
								grid.request(worker[wid + 1], {cmd: 'action'},
						...
						if (jobFinished)
							grid.send(master_uuid, {cmd: 'endJob', data: stageData.action.jobId});
					} else {
						try {
	...
	this.processAction = function () {
		if (action.finished) {
			...
			action.reset();
			if (worker[wid + 1])
				grid.request(worker[wid + 1], {cmd: 'action'}, function(err) {if (err) throw new Error(err);});
		}
