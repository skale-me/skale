...
module.exports.UgridJob = function(grid, app, param) {
	...
	for (var i = 0; i < param.stageData.length; i++) {
		...
		// Instanciate transformation nodes and sources
		for (var l = 0; l < param.stageData[i].lineages.length; l++) {

			for (var j = 1; j < param.stageData[i].lineages[l].length; j++) {
				if (this.node[param.stageData[i].lineages[l][j]].transform)
					this.node[param.stageData[i].lineages[l][j]] = 
						new RDD[this.node[param.stageData[i].lineages[l][j]].transform](grid, app, this, this.stage[i], this.node[param.stageData[i].lineages[l][j]]);
			}
		}
	}
	// console.log(this.node)
	// Instanciation des noeuds du graph
	/*
	this.node = {};
	for (var i in param.node) {
		var type = param.node[i].type;
		var stage = this.stage[param.node[i].sid];
		var param = param.node[i];
		this.node[i] = new RDD[type](grid, app, job, stage, param);
	}
	*/
...
function Stage(grid, app, job, param) {
	...
	// toute cette boucle remonte dans ugrid job
	for (var l = 0; l < lineages.length; l++)
		this.source[l] = new UgridSource[job.node[lineages[l][0]].type](grid, app, job, this, job.node[lineages[l][0]]);

	// plus besoin e la structure de donnée this.source
	this.run = function () {
			...
			try {
				this.source[l].run(function() {
					...
					if (!param.isLastStage) {
						job.node[shuffleNum].tx_shuffle();
...
function Source(grid, app, job, stage, node) {
	...
	if (node.persistent) {
		...
		try {fs.mkdirSync(dataset_dir);} catch (e) {};
	}
...
var UgridSource = {
	parallelize: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
	...
	stream: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
	...
	fromRAM: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
		var self = this;
	...
	fromSTAGERAM: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
		...
		this.run = function(callback) {
			...
			// on décrémente ici seulement si les stage d'avant sont terminés
			stage.next_target_cnt--;
			callback();
		};
	},
	randomSVMData: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
	...
	textFile: function(grid, app, job, stage, node) {
		Source.call(this, grid, app, job, stage, node);
		...
		function processBlock(bid) {
			...
			function shuffleLine(shuffledLine) {
				...
				if (shuffleTo != app.wid) {
					grid.send(app.worker[shuffleTo].uuid, {cmd: 'lastLine', args: {lastLine: shuffledLine, sid: stage.sid, lid: node.lid, bid: blocks[bid].bid + 1}, jobId: job.id});
		...
		function processFirstLine(bid) {
			...
			if (targetBlock.forward && targetBlock.shuffleLastLine) {
				...
				if (targetBlock.shuffleTo != app.wid) {
					grid.send(app.worker[targetBlock.shuffleTo].uuid, {cmd: 'lastLine', args: {lastLine: shuffledLine, sid: stage.sid, lid: node.lid, bid: bid + 1}});
