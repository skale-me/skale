	...
	function Stage (stageData) {
		...
		var tx_shuffle_lib = {
			sample: function (node) {
				...
				state.nShuffle += worker.length - 1;
			},
			groupByKey: function () {
				SRAM = [];
			...
			join: function () {				// ICI SRAM est un objet
				var map = worker.map(function() {return {};});						// Prepare data to be shuffled to each worker
				for (var p in res.v)												// Hash coding on keys to map to workers
					map[ml.cksum(p) % worker.length][p] = res.v[p];
				for (var i = 0; i < map.length; i++) {								// Loop over map to shuffle to given worker
			...
			distinct: function () {
				for (var i = 0; i < worker.length; i++) {
					if (worker[i].uuid == grid.host.uuid) SRAM = res.v[i];
