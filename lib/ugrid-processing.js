...
/*
	Optim pour eviter les copies inutiles (filter, flatMap)
		Prévoir une tableau fixe pour tmp de type
		tmp = [vect_length, [vecteur]], plutot que de recrer un
		nouveau array a chaque iteration sur les elements des partitions
		Chaque tranformation ne retourne plus un booleen mais
		met à jour la valeur tmp[0]
		le continue loop1 sera alors conditionné sur tmp[0]
	Optim pour eviter les branchement inutiles sur l'action
		prévoir une version de pipeline avec action, et une version sans
	Optim ultime
		compiler le pipeline à la volée pour appeler directement
		les fonctions utilisateurs dans les cas map, filter, mapValues, flatMap

	TODO:
		il faut ajouter la mise en RAM conditionnelle des donnees apres chaque
		transformation, ce qui peut etre couteux

	NB:
		- la fonction poststage doit etre specifique à chaque transformation
		  si c'est une tranfo narrow alors on appelle la callback finale
		  sinon on appelle tx_shuffle
		- 3 types de mémoires: 
			- RAM: stocke les datasets partitionés, 
				--> durée de vie = plusieurs taches
			- STAGE_RAM: stocke les sous-parties de datasets partitionnés
				--> durée de vie = un stage
			- res: stocke les données brutes, pas forcément partitionnées
				à passer à la fonction tx_shuffle chargée de la mise en forme 
				et de l'envoie aux workers
		- Changer le nom res en SWAP, plus explicite ?
*/
...
var API = {
	fromRAM: function (STAGE_RAM, RAM, state, node, res, trans, callback, action) {
		...
		for (var p in input) {
			...
			for (var i = 0; i < partition.length; i++) {
				tmp = [partition[i]];
				for (var t = 1; t < trans.length; t++)
					if (fun[t](trans[t], node, tmp, p, res) == false)
						continue loop1;
	...
	sample: function (trans, node, tmp, p, res) {
		...
		var len = res.len;
		for (var i = 0; i < tmp.length; i++) {
			len[p]++; // Incremente le nombre d'elements reçus pour la partition en cours
	...
	join: function (trans, nodes, tmp, p, res) {
		...
		for (var i = 0; i < tmp.length; i++) {
			...
			if (dest[key] == undefined)
				dest[key] = {};
			if (dest[key][src_id] == undefined)
				dest[key][src_id] = [];
			dest[key][src_id].push(value);
		}
		return true;
	},
