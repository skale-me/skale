...
var API = {
	...
	flatMap : function (trans, nodes, tmp) {
		var node = nodes[trans.num];
		var data = tmp.v;
		var t0 = [];
	...
	groupByKey: function (trans, nodes, tmp, dummy, res) {
		var data = tmp.v;
		var dest = res.v;		
		for (var i = 0; i < data.length; i++) {
			var key = data[i][0];
	...
	reduceByKey: function (trans, nodes, tmp, dummy, res) {
		var node = nodes[trans.num];
		var data = tmp.v;
		var dest = res.v;
	...
	union : function (trans, nodes, tmp) {},
	join: function (trans, nodes, tmp, dummy, res) {
		var data = tmp.v;
		var dest = res.v;
	...
	coGroup: function (trans, nodes, tmp, dummy, res) {
		var data = tmp.v;
		var dest = res.v;
		var src_id = trans.src_id;
		for (var i = 0; i < data.length; i++) {
	...
	crossProduct: function (trans, nodes, tmp, dummy, res) {},
	mapValues : function (trans, nodes, tmp) {
	...
	count: function (action, res, tmp) {res.v += tmp.v.length;},
	...
	reduce: function (action, res, tmp) {
...
function Stage(grid, STAGE_RAM, RAM, stageData, worker, node) {
	...
	this.run = function (callback) {
		...
		if (!state.locked && (lineages.length == state.cnt)) callback(res.v);	// Call poststage
	}
