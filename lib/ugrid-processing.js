...
function UgridTask(grid, RAM, msg) {
	...
	function Stage (stageData) {
		...
		function Lineage (transform, callback) {
			...
			function pipeline (p) {
				for (var t = 1; t < transform.length; t++) {					
					tmp = node[transform[t].num].transform.pipeline(tmp, p, transform[t].src_id, worker);					
			...
			var source = {
				fromRAM: function () {
					...
					if (fun_action) callback(fun_action.result);
						...
						try {
							node[transform[transform.length - 1].num].transform.tx_shuffle(state, worker, grid);
							callback();
						} catch (err) {
		...
		this.shuffle = function (msg, callback) {
			try {
				node[shuffleNum].transform.rx_shuffle(msg.data.args, state, worker);
