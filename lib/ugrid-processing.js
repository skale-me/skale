		...
		if (sid == (nStage - 1)) {
			if ((app.wid === 0) || (++action.semaphore == 2)) {
				if (!ignore) {
					...
					action.semaphore = 0;					
					/*
						ici il faut traverser tous les noeuds du graphe et reseter toutes les transformation
						ensuite il faut boucler sur le state de tous les stages et obsoletiser tout ceux
						dont le next_target_cnt est egal à 0 et setter stageIdx au premier dont le next_target_cnt n'est pas nul
					*/
					if (app.worker[app.wid + 1])
						grid.send(app.worker[app.wid + 1].uuid, {cmd: 'action', jobId: param.jobId});
				}
