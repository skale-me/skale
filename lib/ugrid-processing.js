			...
			for (var i = 0; i < partition.length; i++) {
				...
				for (var t = 1; t < trans.length; t++) {
					if (fun[t](trans[t], node, tmp, p, res) == false)
						continue loop1;
					if (trans[t].persistent) {
						if (RAM[trans[t].dest_id] == undefined)
							RAM[trans[t].dest_id] = {};
						if (RAM[trans[t].dest_id][p] == undefined)
							RAM[trans[t].dest_id][p] = tmp.v;
						else
							RAM[trans[t].dest_id][p] = RAM[trans[t].dest_id][p].concat(tmp.v);
					}
				}
