...
function UgridTask(grid, RAM, msg) {
	...
	function Stage (stageData) {
		...
		var tx_shuffle_lib = {
			sample: function (node) {
				...
				for (var i in res.v) {
					...
					STAGE_RAM.v[p] = {data: []};
					var idxVect = [];
					while (STAGE_RAM.v[p].data.length != L) {
						...
						idxVect.push[idx];
						STAGE_RAM.v[p].data.push(res.v[i][idx]);
					}
					p++;
				}
		...
		var rx_shuffle_lib = {
			...
			crossProduct: function (node, msg, callback) {
				var data = msg.data.args;
				for (var i in data) {
					if (i in STAGE_RAM.v) {
						for (var j = 0; j < data[i].length; j++)
							STAGE_RAM.v[i].push(data[i][j]);
					} else STAGE_RAM.v[i] = data[i];
				}
				grid.reply(msg, null, 'Shuffle done by worker ' + msg.id);
				callback();
		...
		var post_shuffle_lib = {
			...
			coGroup: function (node) {
				...
				for (var i in STAGE_RAM.v) {
					...
					if (datasets.length != 3) continue;
					res.push({
						key: STAGE_RAM.v[i].key,
						data:[[STAGE_RAM.v[i].key, [STAGE_RAM.v[i][node.child[0]], STAGE_RAM.v[i][node.child[1]]]]]
					})
				}
				STAGE_RAM.v = res;
			},
			crossProduct: function (node) {
				if (Object.keys(STAGE_RAM.v).length == 0) return;
				var res = [];
				...
				var a = STAGE_RAM.v[node.child[0]];
				var b = STAGE_RAM.v[node.child[1]];
