...
function Stage(grid, STAGE_RAM, RAM, stageData, worker, node) {
	...
	for (var i in node) {
		if (node[i].src)
			node[i].src = eval('node[i].src = ' + node[i].src.toString());
	}
	if (action && action.src)
		action.src = eval('action.src = ' + action.src.toString());
	if (action && action.post_src)
		action.post_src = eval('action.post_src = ' + action.post_src.toString());
