...
function Stage(grid, STAGE_RAM, RAM, stageData, worker, node) {
	...
	function Lineage (lid, transform, callback) {
		...
		function pipeline (p) {			
			for (var t = 1; t < transform.length; t++) {				
				...
				if (transform[t].persistent && (transform[t].dependency == 'narrow')) {
					...
					if (RAM[transform[t].dest_id][p] == undefined) RAM[transform[t].dest_id][p] = tmp;
					else RAM[transform[t].dest_id][p] = RAM[transform[t].dest_id][p].concat(tmp);
				}
			}
			action && fun_action(p);
		}
		...
		source[transform[0].type](callback);	// Run lineage
	}

	this.run = function (callback) {
		for (var l = 0; l < lineages.length; l++) {
			try {new Lineage(l, lineages[l], callback);} 
			catch (e) {throw e;}
		}
	}
