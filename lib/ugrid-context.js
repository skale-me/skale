...
function UgridContext(arg) {
	...
	this.randomSVMData = function (N, D, seed, nPartitions) {
		var a = new UgridArray(self, [], 'RandomSVMData', {D: D});
		a.getSplits = function(callback) {
			...
			var split = [];
			for (var i = 0; i < worker.length; i++) split.push(data[i]);
			callback(split);
		};
		return a;
	};
	...
	this.parallelize = function (localArray, nPartitions) {
		if ((localArray == undefined) || (localArray == null) || !(localArray instanceof Array))
			throw new Error('First argument of function parallelize() must be a instance of class Array.');
	...
	this.textFile = function (file, nPartitions) {
		var a = new UgridArray(self, [], 'TextFile');
		a.getSplits = function(callback) {
			...
			function mapLogicalSplit(split) {
				...
				var args = [];
				for (var i = 0; i < worker.length; i++) args.push(split);
				callback(args);
			}
		};
		return a;
	...
	function stream(inputStream, config, streamType, streamid) {
		var a =  new UgridArray(self, [], 'Stream', {streamId: streamid});
