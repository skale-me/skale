...
function UgridContext(arg, callback) {
	...
	function stream(inputStream, config, streamType) {
		...
		var dispatch = a.dispatch = new DispatchStream(self, streams, mapN, config.N);
		a.config = config;
		if (streamType == 'line') {
...
function DispatchStream (self, dests, mapN, N) {
...
DispatchStream.prototype._flush = function (done) {
	for (var i in this.dests) {
		var ignore = (this.total == 0) || (i < this.last) || (i >= this.N);

		// WORKAROUND: valable uniquement pour un seul job
		if (ignore && !this.grid.jobs[0].onlyStreams && (this.grid.jobs[0].iteration == 0))
			ignore = false;

		this.dests[i].emit('end', ignore);
	}
	done();
};
