...
function UgridContext(arg) {
	...
	this.stream = function (config) {
		console.log('in Stream');
		var stream  = self.createWriteStream(streamId, worker[0].uuid);
		setInterval(function () {
			console.log('streaming hello world');
			stream.write('hello world');
		}, 100);
		return new UgridArray(self, worker, [], 'narrow', 'stream', [config, streamId++]);
	}
}
