...
function UgridContext(arg) {
	...
	this.stream = function (inputStream, config) {
		...
		var dispatch = new DispatchStream(streams);
...
function DispatchStream (dests) {
	if (!(this instanceof DispatchStream))
		return new DispatchStream(client, name);
...
DispatchStream.prototype._transform = function (chunk, encoding, done) {
	console.log('in dispatch transform');
...
DispatchStream.prototype._flush = function (done) {
	for (var i in this.dests) this.dests[i].end();
	done();
};
