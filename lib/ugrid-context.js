...
function UgridContext(arg) {
	...
	this.textFile2 = function (path, nPartitions) {
		var a = new UgridArray(self, [], 'narrow', 'textFile2', [path, nPartitions || worker.length]);
		a.getArgs = function(callback) {
			var file = a.args[0];
			var u = url.parse(file);
			var size = fs.statSync(file).size;
			...
			var blocks = [];
			var nBytes;
			// on construit un vecteur de block égal au nombre de partition global, pour le moment 1 seule par worker
			var nBlocks = worker.length;
			for (var i = 0; i < nBlocks; i++) {
				if (i == nBlocks - 1) nBytes = size - base * (nBlocks - 1);
				else nBytes = base;
				var opt = {start: i * base, end: i * base + nBytes - 1};
				if (opt.end < opt.start) opt.end = opt.start;		// cette ligne pose probleme
				blocks[i] = {file: file, opt: opt, bid: i, wid: i, semaphore: 0};
			}
