...
function UgridContext(arg) {
	...
	this.basedir = '/tmp/ugrid/' + this.contextId + '/';
	try {fs.mkdirSync('/tmp/ugrid/');} catch (e) {;}
	try {fs.mkdirSync(this.basedir);} catch (e) {;}
	try {fs.mkdirSync(this.basedir + 'tmp/');} catch (e) {;}
	try {fs.mkdirSync(this.basedir + 'stream/');} catch (e) {;}
...
function Task(contextId, jobId, nodes, rddid, pid, action) {
	...
	this.run = function(done) {
		...
		// create context scratch directory if it does not exist
		this.basedir = '/tmp/ugrid/' + this.contextId + '/';
		try {fs.mkdirSync('/tmp/ugrid/');} catch (e) {;}
		try {fs.mkdirSync(this.basedir);} catch (e) {;}
		try {fs.mkdirSync(this.basedir + 'shuffle/');} catch (e) {;}
	...
	this.streamFile = function (file, callback) {
		var fs = this.nodes[this.rddid].fs;
		...
		s.once('error', function () {
			console.log('# get %s from worker %s', file.path, file.host);
			callback(self.grid.createStreamFrom(file.host, {cmd: 'sendFile', path: file.path}));
		});
	};
}
