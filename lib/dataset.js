...
TextAzure.prototype.iterate = function (task, p, pipeline, done) {
  ...
  azureDownload(az, this.container, path, task, pipeline, done);
/*
...
function azureDownload(az, container, name, task, pipeline, done) {
  ...
  var stream = gz ? task.lib.zlib.createGunzip({chunkSize: 65536}) : new task.lib.Lines();

  az.getBlobToStream(container, name, stream, {
    parallelOperationThreadCount: 1,
  }, function (err) {
    task.log('getBlob', name, 'err:', err);
  });

  if (gz) return iterateStream(stream, task, pipeline, done);

  stream.on('data', function (linev) {
    var buffer;
    //task.log('linev len:', linev.length);
