...
function Source(uc, getSplit, args, nPartitions) {
	Dataset.call(this, uc);

	this.getPartitions = function (done) {
		var P = nPartitions || uc.worker.length;
		this.partitions = [];
		this.splits = [];
...
function parallelize(uc, localArray, nPartitions) {
	...
	return new Source(uc, getSplit, localArray, nPartitions);
}

function range(uc, start, end, step, nPartitions) {
	function getSplit(P, p, arg) {
	...
	return new Source(uc, getSplit, arg, nPartitions);
}
...
function Stream(uc, stream, type, config) { // type = 'line' ou 'object'
	var id = uuid.v4();
	var tmpFile = '/tmp/skale/' + uc.contextId + '/tmp/' + id;
	var targetFile = '/tmp/skale/' + uc.contextId + '/stream/' + id;
...
function TextFile(uc, file, nPartitions) {
	...
	this.getPartitions = function(done) {
		...
		if (this.watched) {
			var watcher = fs.watch('/tmp/skale/' + uc.contextId + '/stream', function (event, filename) {
