...
Dataset.prototype.lookup = thenify(function (key, done) {
	return this.filter((kv, args) => (kv[0] === args.key), {key: key}).map(kv => kv[1]).collect(done);
});

Dataset.prototype.countByValue = thenify(function (done) {
	return this.map(e => [e, 1]).reduceByKey((a, b) => a + b, 0).collect(done);
});

Dataset.prototype.countByKey = thenify(function (done) {
	return this.mapValues(() => 1).reduceByKey((a, b) => a + b, 0).collect(done);
});
...
Dataset.prototype.collect = thenify(function (done) {
	...
	return this.sc.runJob(null, this, action, function (job, tasks) {
...
Dataset.prototype.first = thenify(function(done) { return this.take(1, done); });
...
Dataset.prototype.top = thenify(function (N, opt, done) {
	var reducer = function(a, b) {a.push(b); return a;};
...
Dataset.prototype.forEach = thenify(function (eacher, opt, done) {
	return this.aggregate(eacher, () => null, null, opt, done);
});
