...
var skalify  = require('./skalify.js');
...
Dataset.prototype.map = function (mapper, args) {return new Map(this.sc, this, mapper, args);};

Dataset.prototype.flatMap = function (mapper, args) {return new FlatMap(this.sc, this, mapper, args);};

Dataset.prototype.mapValues = function (mapper, args) {return new MapValues(this.sc, this, mapper, args);};

Dataset.prototype.flatMapValues = function (mapper, args) {return new FlatMapValues(this.sc, this, mapper, args);};

Dataset.prototype.filter = function (filter, args) {return new Filter(this.sc, this, filter, args);};

Dataset.prototype.sample = function (withReplacement, frac, seed) {return new Sample(this.sc, this, withReplacement, frac, seed || 1);};
...
Dataset.prototype.aggregateByKey = function(combiner, reducer, init, args) {
	...
	return new AggregateByKey(this.sc, [this], combiner, reducer, init, args);
};
...
Dataset.prototype.groupByKey = function () {
	...
	return new AggregateByKey(this.sc, [this], combiner, reducer, [], {});
};
...
Dataset.prototype.coGroup = function (other) {
	...
	return new AggregateByKey(this.sc, [this, other], combiner, reducer, [], {});
};
...
Dataset.prototype.aggregate = skalify(function (reducer, combiner, init, opt, callback) {
	opt = opt || {};
...
Dataset.prototype.reduce = skalify(function (reducer, init, opt, callback) {
	opt = opt || {};
	if (arguments.length < 4) callback = opt;
	return this.aggregate(reducer, reducer, init, opt, callback);
});

Dataset.prototype.count = skalify(function (opt, callback) {
	opt = opt || {};
...
Dataset.prototype.forEach = skalify(function (eacher, opt, callback) {
	opt = opt || {};
...
function Source(sc, getSplit, args, nPartitions) {
	Dataset.call(this, sc);
	this.nPartitions = nPartitions;
	this.getSplit = getSplit;
	this.getSplitArgs = args;

	this.iterate = function (task, p, pipeline, done) {
		var buffer;
		this.splits[p] = this.getSplit(this.partitions.length, p, this.getSplitArgs, task);
...
function parallelize(sc, localArray, nPartitions) {
	...
	function getSplit(P, p, arg) {
		var size = Math.ceil(arg.length / P);
		return arg.slice(p * size, (p + 1) * size);
	}

	return new Source(sc, getSplit, localArray, nPartitions);
}

function range(sc, start, end, step, nPartitions) {
	function getSplit(P, p, arg) {
		var n = Math.ceil(((arg.end - arg.start) / arg.step) / P);
		var v = arg.start + p * n * arg.step;
		var i = 0, out = [];

		if (arg.end > arg.start)
			while (i < n && v < arg.end) { out[i++] = v; v += arg.step; }
		else
			while (i < n && v > arg.end) { out[i++] = v; v += arg.step; }
		return out;
	}
	...
	var arg = {
		start: end === undefined ? 0 : (start || 0),
		end: end === undefined ? (start || 0) : end,
		step: step || 1
	};

	return new Source(sc, getSplit, arg, nPartitions);
}
...
function Stream(sc, stream, type, config) { // type = 'line' ou 'object'
	var id = uuid.v4();
...
function Map(sc, parent, mapper, args) {
	Dataset.call(this, sc, [parent]);
	this.mapper = mapper;
	this.args = args;
...
function FlatMap(sc, parent, mapper, args) {
	Dataset.call(this, sc, [parent]);
	this.mapper = mapper;
	this.args = args;
...
function MapValues(sc, parent, mapper, args) {
	Dataset.call(this, sc, [parent]);
	this.mapper = mapper;
	this.args = args;
...
function FlatMapValues(sc, parent, mapper, args) {
	Dataset.call(this, sc, [parent]);
	this.mapper = mapper;
	this.args = args;
...
function Filter(sc, parent, filter, args) {
	Dataset.call(this, sc, [parent]);
	this.filter = filter;
	this.args = args;
...
function Sample(sc, parent, withReplacement, frac, seed) {
	Dataset.call(this, sc, [parent]);
...
function AggregateByKey(sc, dependencies, combiner, reducer, init, args) {
...
function PartitionBy(sc, dependencies, partitioner) {	// nécessairement sur un clé valeur
	Dataset.call(this, sc, [dependencies]);
	this.shuffling = true;
	this.executed = false;
	this.buffer = [];
