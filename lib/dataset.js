...
TextS3File.prototype.iterate = function (task, p, pipeline, done) {
  ...
  if (this.path.slice(-8) === '.parquet') return parquetStream(rs, this.path, task, pipeline, done); 
...
TextS3Dir.prototype.getPartitions = function (done) {
  ...
  getList([], null, function (err, res) {
    //self.sc.log('TextS3Dir list:', res);
...
TextS3Dir.prototype.iterate = function (task, p, pipeline, done) {
  ...
  if (path.slice(-8) === '.parquet') return parquetStream(rs, path, task, pipeline, done);
...
TextDir.prototype.iterate = function (task, p, pipeline, done) {
  ...
  if (path.slice(-8) === '.parquet') return parquetIterate(path, pipeline, done);
  var tail = '';
