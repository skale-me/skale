...
function Stream(sc, stream, type, config) { // type = 'line' ou 'object'
	var id = uuid.v4();
	var tmpFile = '/tmp/ugrid/' + sc.contextId + '/tmp/' + id;
	var targetFile = '/tmp/ugrid/' + sc.contextId + '/stream/' + id;
...
function TextFile(sc, file, nPartitions) {
	...
	this.getPartitions = function(done) {
		...
		if (this.watched) {
			var watcher = fs.watch('/tmp/ugrid/' + sc.contextId + '/stream', function (event, filename) {
