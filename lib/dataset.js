...
function TextAzureFile(sc, file, options) {
  Dataset.call(this, sc);
  var _a = file.split('/');
  this.container = _a[0].replace(/@.*/, '');
  this.path = _a.slice(1).join('/');
  this.type = 'TextAzureFile';
  this.options = options || {};
}

util.inherits(TextAzureFile, Dataset);

TextAzureFile.prototype.getPartitions = function (done) {
  this.partitions = {0: new Partition(this.id, 0)};
  this.nPartitions = 1;
  done();
};

TextAzureFile.prototype.iterate = function (task, p, pipeline, done) {
  var az = azure.createBlobService();
  var rs = az.createReadStream(this.container, this.path, null);
  var tail = '';

  task.log('stream azure', this.container, this.path);
  if (this.options.parquet || this.path.slice(-8) === '.parquet')
    return parquetStream(rs, this.path, task, pipeline, done);
...
function TextAzureDir(sc, dir, options) {
  Dataset.call(this, sc);
  var _a = dir.split('/');
  this.container = _a[0].replace(/@.*/, '');
  this.prefix = _a.slice(1).join('/');
  this.type = 'TextAzureDir';
  this.options = options || {};
  this.options.azure = this.options.azure || {};
}

util.inherits(TextAzureDir, Dataset);

TextAzureDir.prototype.getPartitions = function (done) {
  var self = this;
  var az = azure.createBlobService();
  ...
  getList([], null, function (err, res) {
    ...
    for (var i = 0; i < self.nPartitions; i++) {
      ...
      self.partitions[i].path = res[i].name;
    }
...
TextAzureDir.prototype.iterate = function (task, p, pipeline, done) {
  task.log('azure options:', this.options.azure);
...
module.exports = {
  ...
  TextAzureDir: TextAzureDir,
  TextAzureFile: TextAzureFile,
  Source: Source,
  Stream: Stream,
  Random: Random,
  Map: Map,
