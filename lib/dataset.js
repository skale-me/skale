...
Dataset.prototype.aggregate = thenify(function (reducer, combiner, init, opt, done) {
	...
	return this.sc.runJob(opt, this, action, function(job, tasks) {
		...
		function runBatch(ntasks, index, done) {
			var cnt = 0, tmax = index + ntasks - 1, prev;
			...
			for (var i = 0; i < ntasks; i ++) {
				self.sc.runTask(tasks[index + i], function (err, res) {
					...
					index += ntasks;
					prev = ntasks;
					ntasks = (tasks.length - index) < nworker ? (tasks.length - index) : nworker;
					self.sc.log('finished ' + prev + ' result tasks, remain:', ntasks);
...
function TextFile(sc, file, nPartitions) {
	...
	this.getPartitions = function(done) {
		...
		if (this.watched) {
			var watcher = fs.watch(sc.basedir + 'stream', function (event, filename) {
