...
Dataset.prototype.save = thenify(function (filename, done) {
  var reducer = function (acc, val, opt, wc) {
    acc = acc.concat(val.toString() + '\n');
    if (acc.length >= 65536) {
      wc.lib.fs.appendFileSync(opt.filename, acc);
      acc = '';
    }
    return acc;
  };
  var flush = function (acc, opt, wc) {
    wc.lib.fs.appendFileSync(opt.filename, acc);
  };
  // Parallel save
  // return this.aggregate(reducer, function () {}, '', {filename: filename, _postIterate: flush}, done);

  // Serial save
  var action = {args: [], src: reducer, init: '', opt: {filename: filename, _postIterate: flush}}, self = this;

  try { fs.unlinkSync(filename); } catch (err) { null; }
...
Dataset.prototype.aggregate = thenify(function (reducer, combiner, init, opt, done) {
  opt = opt || {};
  var action = {args: [], src: reducer, init: init, opt: opt}, self = this;
  ...
  return this.sc.runJob(opt, this, action, function (job, tasks) {
    ...
    function runBatch(ntasks, index, done) {
      ...
      self.sc.log('start result tasks', index + '-' + tmax);
      for (var i = 0; i < ntasks; i ++) {
        self.sc.runTask(tasks[index + i], function (err, res) {
          result = combiner(result, res.data, opt);
          if (++cnt < ntasks) return;
          index += ntasks;
          ntasks = tasks.length - index;
          self.sc.log('finished result tasks:', index + '/' + tasks.length);
...
GzipFile.prototype.iterate = function (task, p, pipeline, done) {
  var rs = task.lib.fs.createReadStream(this.file).pipe(task.lib.zlib.createGunzip());
  var tail = '';
...
TextFile.prototype.getPartitions = function (done) {
  ...
  function getSplits() {
    ...
    if ((u.protocol == 'hdfs:') && u.slashes && u.hostname && u.port) splitHDFSFile(u.path, self.nSplit, mapLogicalSplit);
    else splitLocalFile(u.path, self.nSplit, mapLogicalSplit);
...
Cartesian.prototype.getPartitions = function (done) {
  if (this.partitions == undefined) {
    this.pleft = this.dependencies[0].partitions.length;
    this.pright =  this.dependencies[1].partitions.length;
...
SortBy.prototype.getPartitions = function (done) {
  if (this.partitions == undefined) {
    var P = Math.max(this.numPartitions || 1, this.dependencies[0].partitions.length);
